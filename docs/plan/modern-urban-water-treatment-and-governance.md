...
都市の予算用途を現地の為政者・管理担当者が決められる形としては、**自治憲章を持つ首長主導型の自治都市**を標準形とする。市長、ポデスタ、市政官、都市代官、または任命制の水道委員長が、承認済みの「都市公共事業枠」の内訳を水道・下水・道路・市場から選べる。税率変更、起債、他都市の河川へ影響する放流は、議会または State の同意を要する別権限に留める。
...
| Phase | 変更 | 既存資産との接続 |
| --- | --- | --- |
| 1 | `ModernWaterTreatmentSystem`、取水・放流の地理判定、`waterSecurity` / `riverPollutionLoad` の分離 | `UrbanWaterSystem`、疫病水質設計 |
| 2 | 水源保護、低速砂濾過、一次沈殿、建設費と運転費の分離 | Burg treasury、清掃税・接続料 |
| 3 | 都市自治権限（`utilityBudgetAuthority`）、水道委員長、公共事業枠の UI | Burg/State の行政按分、既存 fiscal report |
| 4 | 凝集・急速濾過・`Chlorine` 消費・水質検査 | era 6 の化学工業、`Chlorine` Good |
| 5 | 散水ろ床／活性汚泥、汚泥処理、流域補償、放流水検査 | `sanitaryEngineering`、上流・下流汚染外交 |
