...
**Phase 1a・Phase 1b ともに実装完了(2026-08-13)。** Phase 2は却下(理由は下記および3節)。Phase 3は将来必要になった場合のみ着手。
...
- **Phase 2は却下**（ユーザー指摘、2026-08-13）: 「ループの固定オーバーヘッド削減」はP2-5の不変条件を保ったまま365回のイテレーションを減らせないことが前提の対策だが、その制約はまさにPhase 3が明示的に外すもの。Nが大きい場面はPhase 3に任せられ、Nが小さい場面は元々効果が誤差レベル、Phase 3が効かない`autonomous`大量advanceも支配的コストは対象外のnobility.tickなのでPhase 2では解決しない——結局どのシナリオでも実装リスク（day-batch/RNG/TransactionWriterという全経路共有のインフラを触る）に見合わない。詳細は3節Phase 2。
...
### Phase 2（却下・2026-08-13） — ループ自体の固定オーバーヘッド削減

**当初案**: 「ループそのものの固定コスト」（regiment状態リセット、暦計算、RNGストリーム切替、`TransactionWriter`生成、day-batchスナップショット/commit/通知）を、「365回のコミット自体は残したまま、各回の固定費を削る」方向で削減する。P2-5の不変条件には触れない、比較的安全な方向。

**却下理由**（ユーザー指摘、2026-08-13）: Phase 2が必要なのは「P2-5の不変条件を保ったまま365回イテレーションする」という制約を外せないからであり、その制約はPhase 3（下記）がまさに明示的に外すもの。Phase 3が存在すれば、Phase 2の削減対象だった「大きなNに対する365回×固定費」というシナリオ自体をPhase 3側に逃がせるため、Phase 2の価値が構造的に失われる。実際に精査すると:

- **Nが大きい場合**（Phase 2の効果が意味を持ちうる唯一のケース）: 日次精度を求めないなら最初からPhase 3を使うべきで、Phase 2は不要。
- **Nが小さい場合**（Advance Day/Week程度）: 元々の見積もり通り効果は「数百ms/年」程度で誤差レベルであり、Phase 3の有無に関わらず優先度は最初から低い。
- **Phase 3が効かない残りケース**（`autonomous`ポリシー下の大量advance、Phase 3のNobility/AI扱いが別途要検討で未定のため）: このケースで支配的なコストは`nobility.tick`自体の日次AI処理（~11ms/日≒4s/年、2.3節の通り意図的にバッチ化対象外）であり、Phase 2が触る「ループの固定費」はその一部（数百ms/年）に過ぎない。つまりこのケースが重い問題は、Phase 2を実装してもほぼ解決しない。

以上、Phase 2はどのシナリオでも「共有インフラ（day-batchスナップショット・RNGストリーム切替・`TransactionWriter`など、全経路が通る箇所）を触るリスク」に見合う価値を持たないと判断し、**着手しない**。将来、真に大きな一括進行が必要になった場合はPhase 3（下記）で設計する。

### Phase 3（本計画の対象外、将来必要になった場合に着手） — 明示的な「Fast-Forward」専用パス
...
| ループの固定オーバーヘッドを削る | ❌ 却下（Phase 2） | Phase 3が同じ問題をより根本的に解決するため価値が構造的に無くなった。どのシナリオでも実装リスクに見合わない |
...
4. ~~Phase 2着手要否の判断。~~ **却下（2026-08-13、ユーザー指摘: Phase 3が同じ問題をより根本的に解決するため不要）**
5. Phase 3（Fast-Forward専用パス）はユーザーから明確な要望が出た場合にのみ、別途ミニ計画を立てる。本計画としてはこれで完了とする。
