...
- **年次ロジック**(`GuildSuccessionModule.settleAnnual()`、`economy.tick`内`GuildKnowledge.settleAnnual()`の直後、docs記載順どおり自己ゲート): stock>0のBurgごとに(1)既存マスターが死亡していれば弟子がいれば昇格・いなければ`applyMasterlessGuildPenalty()`(新規、`guildKnowledge.ts`、一回限り-30%の"secrets were lost"ペナルティ、通常の年次減衰`GUILD_DECAY_RATE`とは別の定数)、死亡親方の個人資産(`Character.wealth`)は後継者へ75%を引き継ぎ、後継者不在時は30%をギルド金庫へ戻す。ギルド金庫そのものは制度資産のため相続しない。(2)マスター不在なら`createPerson`で新規生成(`marketManagers.ts`と同型)し、bootstrap済みのギルド金庫から小額の初期資産を支給する。(3)既存弟子の`engineering`スキルを`masterSkill/100 × GuildKnowledgeStock.stock × APPRENTICE_MAX_ANNUAL_GROWTH`で成長、(4)マスターの`engineering`が閾値(40)以上かつ弟子2人未満なら新規弟子を生成(`ageOverride`で12-17歳の若年キャラクターとして)。
