...
> **燃料モデルの現行仕様**: 本書の初期案にある「石炭と木炭をまとめた Fuel」は、後続の
> [鉱石精錬・治安システム設計](./mineral-smelting-security-system.md#22-燃料副産物の時代区分phase-e実装済み)
> Phase E で置き換え済みである。`Charcoal` は Wood 由来で中世精錬に使う還元材、`Coal` は採掘される
> 鉱物石炭、`Coke` は前工業化以後の将来候補として扱う。以下の旧来の Fuel 記述は、この区分に従って読む。
...
- Sulfur: 火薬の原料
- Fuel: 中世の精錬は主に木炭。鉱山排水・精錬・高炉は燃料と水利を必要とし、石炭／コークスへの移行は後の技術段階とする
...
| 火薬・高炉 | 発破、揚水、スタンプ、焙焼、高炉・精錬炉 | 木炭、硫黄、硝石、技術者（前工業化以後は石炭→コークス） | 大規模鉄、砲、深部の低品位鉱 |
...
Saltpeter + Sulfur + Charcoal → Gunpowder Good
Iron/Bronze + Charcoal → Artillery Good
```
...
- 燃料到達性は立地係数として残すが、精錬所の燃料実需は後続の Phase E で `Charcoal` 市場在庫の消費として実装済みである。Wood 採取は `forestStock` を減らすため、木炭経由で森林と競合する。
...
- ~~石炭と木炭を区別し、森林枯渇システムとどの深さで接続するか~~ — 実装済み。残る検討は、`forestStock` の低下を立地係数 `fuelAccess` にいつ反映させるか、および前工業化後の `Coal → Coke` の導入条件である
