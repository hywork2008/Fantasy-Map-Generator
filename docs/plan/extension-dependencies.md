...
CSVの編集単位と実行時モジュールの所有権は分ける。正本の `docs/plan/data/races.csv` / `races-relations.csv` は一元管理し、`scripts/races/` の検証・変換処理が所有者別のJSONを生成する。統合スキーマもビルドツール専用であり、コア・拡張の実行時コードからimportしない。

| 所有者 | 型・生成物 | 内容 |
| --- | --- | --- |
| コア | `src/data/raceDefinition.ts`, `src/data/races.generated.json` | ID順・キー・寿命・生殖・環境耐性・通常の超自然能力・外交姿勢・混住率・人名圏。既存のセーブ補完が管理する外見基準・外見範囲・美的基準・外見候補も共有データとして維持 |
| Characters | `src/extensions/characters/data/raceTypes.ts`, `races.generated.json`, `raceRelations.generated.json` | 技能・性格補正、先祖返り、婚姻例外、肉食動物、親の参照、隷属人物の役割と出現率、種族間の美的評価 |
| Economy | `src/extensions/economy/data/raceTypes.ts`, `races.generated.json` | 初期財産の蓄積率、上下水道技術の補正 |

通常の超自然能力はコアの `arcaneHuntAssist.ts`、外交姿勢と人名圏は `cultures-generator.ts` でも使うためコア所有とする。先祖返りの超自然プロフィールはCharacters専用。コアのbound判定は外交姿勢から取得し、Charactersの隷属人物の設定に依存しない。

実行時は `app.ts` が `raceService` を `ExtensionAPI.races` に組み込み、`src/extensions/index.ts` の既存の起動経路から各 `init(api)` へ渡す。各拡張のcontext初期化が `hostRaces.ts` にそのサービスをバインドする。動的ZIPでも `init(api)` を通るため、組み込み拡張と同じ契約を使う。

- 拡張はコアの種族カタログを直接importしない。コアへの問い合わせは注入された `ExtensionAPI.races` を使用する。
- `hostRaces.ts` はホスト実装の再exportではなく、型だけを参照するAPIアダプター。ホストが管理する可変状態の別インスタンスをバンドルしない。
- 各拡張の専用設定は自分の生成物から読む。CharactersとEconomy間の静的データ共有のために依存宣言を増やさない。
- コア向けの公開型はCharactersやEconomyの型を継承しない。統合型は `scripts/races/raceParameters.ts` にのみ存在する。
- サービスはカタログ・種族オブジェクトをコピーして返すため、拡張から返却値を編集してもホストのデータは変更されない。世界の変更は従来のコマンド・状態管理経路で行う。
- `export` は所有者別の生成物をキーで結合する。欠けた種族・重複キー・余分な種族を黙って捨てない。ハイブリッドの親指定も保持する。

検証には `npm run races:check`、`npm run lint:architecture`、`npm run test:race-bundles` を用いる。境界lintは種族カタログの直接import・再export・動的import・別拡張の種族データ参照、実行時からビルドツールへの参照を禁止する。型のみの参照とテストの横断検証は許可する。単体バンドル検証はCharacters/Economyの種族データとAPIアダプターのモジュールグラフにホストや別拡張が入らないことを確認する。

`npm run build:economy` と `npm run build:characters` で各拡張の入口もビルドできる。ただし拡張全体には以前からの `hostCore.ts` などを介した静的依存が残っている。この修正は種族CSVの所有権と参照経路を分離するもので、全ドメインの既存依存の撤去まで完了したことを意味しない。
