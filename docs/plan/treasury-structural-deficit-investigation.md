...
**調査・是正ともに完了（2026-09-06）。** 方針A（維持費率の適正化）を実装。方針Bは**実装しないことに決定**
（ユーザー指示、2026-09-06）——本調査はここで区切りとする。「維持費不足による停滞」（崩壊リスク・死亡率
結合など§8.3/§8.4の新規挙動）はユーザー指示によりスコープ外として実装しない。この結果は
`docs/plan/advance-time-fast-forward.md`§5.2の「標準」プリセット国庫成長率確定に反映済み。
...
3. ✅ ~~案Aの実装~~ → **完了（2026-09-06、§9.1）。「維持費不足による停滞」効果（§8.3〜8.5）はユーザー指示に
   よりスコープ外のまま**
4. ✅ ~~案Bに進むか、残存赤字（平均-19%/yr・中央値-13%/yr、§9.3）を許容して次段階に進むかをユーザーに確認~~
   → **案Bは実装しないことに決定（2026-09-06、ユーザー指示）。本調査はここで区切りとする。**
5. ✅ ~~`docs/plan/advance-time-fast-forward.md`§5.2の「標準」プリセット国庫成長率を確定させる~~ →
   **完了（2026-09-06）。案Aのみ適用後の中央値-12.88%（丸めて-13%）を採用した——理由（平均ではなく中央値を
   使った根拠を含む）は同ドキュメント§5.2参照。**

**本調査ドキュメントとしてはここで完了とする。** 案B（州単位の集約予算上限）は将来的に再検討したくなった
場合のための設計論点として§7・§8.6に残しておくが、着手の予定はない。§8.3（死亡率結合）・§8.4（崩壊
リスク・自然状態への回帰）・§8.5（運営費/補修費の分離）も同様にスコープ外のまま記録として残す。
