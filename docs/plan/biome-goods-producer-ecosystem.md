...
2. **決定（実装済み）**: `Grapes`を独立した交易可能な新規Goodとして新設し、生食・`Raisins`（干しブドウ、保存食）・`Wine`の3方向に加工する（§5.3）。3方向の実際の配分比率自体（どれだけ生食に残しどれだけ加工へ回すか）は、Phase 4時点では専用ロジックを持たず、既存の汎用Market需要/価格シグナルに委ねている——durability比例の専用平滑化はPhase 5の範囲（§8）。
