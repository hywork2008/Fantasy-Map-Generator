...
**2026-08-06 Phase 3実装完了**: §8のPhase 3（牧畜、`husbandry.ts`）を実装した。事前にユーザーへ「牧夫1人あたり許容頭数（牧羊犬なしの基礎値）」を確認し、「推定値を使う（羊200頭/牛60頭、推奨）」の回答を得たうえで実装した（§9決定7、§10.3）。新規`src/extensions/economy/generators/husbandry.ts`が、§5.4の設計どおり開放牧草地species（Cattle/Sheep/Goats/Horses/Camels、`isGrazedLivestockGood()`）に限定して、牧草地面積（バイオームタグ別上限係数×地形係数、耕地/Burg面積を除外）×牧夫労働充足率から実際の利用面積を求め、それに種別生態密度を掛けて`getGrazedCarryingCapacity()`を算出する。必要牧夫数（`calculateHusbandryDemand`）はグレイズド種ごとの生需要を、そのセルの`Dogs`個体群ストックから導いた牧羊犬乗数（Sheep/Goats×10、Cattle/Horses/Camels×8、Arnott et al. 2014の上限に整合）込みの実効許容頭数で割って合算する。**配線**: (1) `ruralOccupationAllocation.ts`の§3.1貪欲配分ループへ、漁業・ブドウ栽培(Wine)と並ぶ3つ目の候補として`husbandry`を追加（`husbandryWorkers`/`husbandryRequiredWorkers`列を新設）。(2) `production-utils.ts`のliveAnimal継続生産で、グレイズド種のみ`getHusbandryWorkerFactor()`を掛けてから`drawDomesticatedFaunaOfftake()`へ渡す（Wine同様、`ruralEcosystemDetail`に関わらず常時有効）。(3) `faunaPopulation.ts`の`getDomesticatedCarryingCapacity()`はグレイズド種のみ`husbandry.ts`の値を使うよう分岐し、非グレイズド種（Pig/Chicken/Cats/Dogs）は引き続きPhase 1/2の暫定プロキシを使う。(4) `getWildCarryingCapacity()`のwildHabitatAreaから`getPastureAreaUsedHectares()`を新たに差し引く。**循環importの回避**: `husbandry.ts`は`faunaPopulation.ts`からimportされる側（グレイズド種capacity分岐用）なので、逆方向のimportを避けるため、Dogs個体群ストックの読み取りは`faunaPopulation.ts`のエクスポート関数を経由せず、`economyContext.ts`の`getOrCreateFaunaStockTable()`を直接叩く形でストックキー形式（`cellId:speciesKey`）だけを重複実装した（`ruralOccupationAllocation.ts`の既存の「Local stand-ins」パターンを踏襲）。`calculatePhysicalAreaHectares`に加え、`calculateTerrainWorkableShare()`（地形の可耕/可牧割合カーブ）と`calculateBurgBuiltAreaHectares()`（旧`faunaPopulation.ts`内のprivate関数を移設）を`agriculturalLandUse.ts`へ集約し、`faunaPopulation.ts`と`husbandry.ts`の両方が共有する。**既存テストの回帰対応**: `faunaPopulation.test.ts`の「食用種はフラットレートプロキシを直接使う」系2テストは、CattleがグレイズドspeciesになったことでPig（非グレイズド）に差し替えて意図を維持した（AGENTS.md §5.1の想定どおりの回帰）。新規単体テスト`husbandry.test.ts`（14件）で、グレイズド判定・需要集計（牧羊犬なし/あり）・値の加重平均・workerFactor・牧草地利用面積・キャリング容量を検証。economy拡張全体870件（856+14）が全てgreen、`tsc --noEmit`（テストファイル込みでも確認）/`lint`/`madge --circular`/`npm run build`もすべてクリーン。Grapes新設（§5.3、Phase 4）は引き続き未着手。

**2026-08-06 Phase 4実装完了**: §8のPhase 4（`Grapes`/`Raisins`新設、ブドウ収穫段階の労働力ゲート、Wineのレシピ切替）を実装した。新規`src/extensions/economy/generators/viticulture.ts`が、husbandry.ts（Phase 3）と同じ「バイオームタグ別上限係数×地形係数」の土地適性方式で`vineyardCeilingArea`を求め、そこに人口比例の「実際に耕作を試みる面積（`desiredArea = min(ceiling, population × 0.5ha）`）」という第2の上限を重ねた——Grainの`cultivableArea`（土地上限）対`cultivatedArea`（人口駆動の実耕作）の二段構成を踏襲したもので、これがないと人口の少ないブドウ適地セルが土地上限だけで非現実的に巨大な労働力需要を要求してしまう（家畜のように既存`biomeOutputByTag`レートから需要を導出できるhusbandry.tsとは異なり、Grapesには参照できる既存レートが存在しないため必要になった追加のバウンド）。必要牧夫ならぬ必要農夫数 = `desiredArea × VINEYARD_LABOUR_DAYS_PER_HECTARE(20) / WORKABLE_DAYS_PER_ADULT(140)`（Grainの`LABOUR_DAYS_PER_HECTARE(30)`より低く、§5.3の「果樹は年間労働がGrainほど集中しない」を反映）。`getViticultureWorkerFactor()`（Phase 1のWine直接ゲート関数を`ruralOccupationAllocation.ts`から`viticulture.ts`へ移設・転用——husbandry.tsが自分のworkerFactorを自前で持つのと同じ理由）が、この必要農夫数に対する充足率でGrapesの収穫量（`getGrapeHarvestOutput` = `areaUsed × GRAPE_YIELD_PER_HECTARE_PER_MONTH(0.03) × riverBonus`）をゲートする。**配線**: (1) `ruralOccupationAllocation.ts`の§3.1貪欲配分ループの「viticulture」候補が、旧来のWine population×rate方式から`calculateViticultureDemand()`（面積/労働ベース）に完全に置き換わった——`viticultureWorkers`/`viticultureRequiredWorkers`列自体は既存のまま再利用。(2) `production-utils.ts`は`good.name === "Wine"`の継続生産分岐を削除し、代わりにGameと同型の「biomeOutputByTagを持たないGoodの特別扱い」として`Grapes`の収穫量を独立に追加するブロックを新設した（Grapesは土地/労働ベースの特殊算出のため`biomeOutputByTag`を持たず、既存の汎用ループには一切現れない）。(3) `Wine`Goodの定義から`distribution`/`chance`/`biomeOutputByTag`を削除し、`recipes: [{ Grapes: 2, Barrels: 1 }]`を追加——既存の`Beer: { Grain, Barrels }`と同型で、加工段階は`production-generator.ts`の汎用`runWorkerLoop`＋`craft`セクターにそのまま乗る（§5.2訂正どおり新規メカニズム不要）。新規`Raisins`Good（`recipes: [{ Grapes: 2 }]`、Stockfish同型）も同じ理由で加工コード追加ゼロ。(4) `faunaPopulation.ts`の`getWildCarryingCapacity()`のwildHabitatAreaから`getVineyardAreaUsedHectares()`を新たに差し引く（§4.2の設計どおり）。**値の再校正（副作用）**: Wineの`value`を5→6へ、Vinegar（`{ Wine: 1 }`レシピを持つ）の`value`を5→6へ引き上げた——`Grapes(2)×2 + Barrels(2)×1 = 6`のレシピ原価をWineの価格が下回らないようにする既存の`goods-generator.test.ts`の不変条件（「レシピは原材料費を下回ってはならない」）を満たすため。Raisinsは新設時点で原価4に対し価格5とし、この不変条件を最初から満たす値にした。Wine値上げの連鎖はVinegarの2レシピ経路（`{Wine:1}`と`{Honey:1}`）・Preserved food・Cheeseの`{..., Vinegar: X}`各経路まで手計算で検算し、Vinegar以外は追加変更なしで不変条件を満たすことを確認済み。**既存テストの回帰対応**: `ruralOccupationAllocation.test.ts`の2件（Wine workerFactorゲート・Wine over Fishの優先度）は、`WINE_GOOD`フィクスチャを`GRAPES_GOOD`（面積/労働モデル向けの`cells.area`/`distanceScale`付き）に差し替えて期待値を再計算した（AGENTS.md §5.1の想定どおりの回帰——husbandry.ts導入時と同型のパターン）。`goods-generator.test.ts`に新規7件（Grapesのカタログ定義・Wineのレシピ形状・Grapes/Raisins/Wineレシピ移行関数の単体テスト、Grapes未移行時のRaisins/Wine移行no-op確認を含む）、新規`viticulture.test.ts`（11件：需要計算のゼロケース・バイオーム非対応・人口比例upper-bound・土地上限クランプ・workerFactor・利用面積・収穫量・河川ボーナス）を追加。economy拡張全体888件（870+18）が全てgreen、`tsc --noEmit`（テストファイル込みでも確認）/`lint`/`madge --circular`/`npm run build`もすべてクリーン。§8のPhase 5（3分岐配分の需要連動＋durability比例の再配分速度）は引き続き未着手——Phase 4時点ではGrapes/Raisins/Wineの実際の産出配分比率は既存の汎用Market需要/価格シグナルに委ねられており、durability比例の専用平滑化ロジックはまだ存在しない。

**2026-08-06 Phase 5実装完了**: §8のPhase 5（3分岐配分の需要連動＋`durability`比例の再配分速度、§9.4）を実装した。実装前に`production-generator.ts`（`makeProductionDecision`/`getDemandEffect`/`smoothCraftWorkers`等）を読解し、以下を確認した——(1) Wine/Raisinsを含むレシピ品全般は、Burg毎に4労働ステップごと再評価する**勝者総取り**方式（`makeProductionDecision`、現在最も`normalizedGain`が高い1品だけが選ばれる）で生産されており、複数候補への按分配分機構は存在しない。(2) `BurgProductionState`（`activeGoalGoodId`含む）は生産サイクルごとに毎回新規生成され、サイクルをまたぐ記憶を一切持たない。(3) 唯一の既存平滑化`smoothCraftWorkers`（craftEmployment.ts）は雇用統計の**読み取り専用レポート集計**であり、`makeProductionDecision`の判定へは一切フィードバックしない。(4) `good.trade.durability`を読むコードは`production-generator.ts`/`craftEmployment.ts`のどこにも存在しなかった。§5.2/§9.5の「既存の汎用パイプラインが既に担っている」という記述自体は正確（レシピ品全般の労働力ゲートに新規`requiredWorkers`機構は不要という主張は今回の調査でも裏付けられた）が、durability比例の再配分速度だけは文字通り「まだ存在しない」ことが確定したため、新規実装が必要と判断した。

**設計判断（「生食Grapesは即応」は実装不要）**: 3分岐のうち生食（未加工のまま残るGrapes）は、WineにもRaisinsにも変換されなかった残りが定義上そのまま販売可能なGrapes在庫になる——「即座に反映される」という挙動が構造上自動的に成立しており、明示的な平滑化ロジックは不要（§5.3の「生鮮Grapesは即応」はこの自動性の言い換えとして達成済みと判断した）。よって本フェーズで新設すべきなのはWineとRaisinsの間の配分速度差だけに絞られた。

**実装（新規`viticultureAllocation.ts`）**: Burgごと・Good（Wine/Raisins）ごとに0..1の「平滑化済み優先シェア」を`simulation.extensions.economy.viticultureAllocationShares`（キー`burgId:goodName`）へ持たせる。`advanceViticultureAllocationShares(burgId, market)`が生産サイクルごとに1回（`produceForBurg`内、`smoothCraftWorkers`呼び出しの直後）呼ばれ、Market在庫の希少性×売価から導いた「その瞬間の目標シェア」へ、`durability`に反比例するレート（`BASE_REALLOCATION_RATE(0.5) / durability`、Wine durability5→レート0.1、Raisins durability4→レート0.125）でEMA的に近づける。`getViticultureAllocationMultiplier(good, burgId)`が、この平滑化シェアを`1 ± 0.6`の範囲のバイアス乗数へ変換し、`makeProductionDecision`内の`demandEffect.multiplier`へ、既存の`strategicDemandMultiplier`と全く同じ挿入パターンで掛け合わせる——Wine/Raisins以外の全Goodには常に1（無効化）を返すため、それ以外のレシピ品の生産判定には一切影響しない。この設計により、`production-generator.ts`本体への変更は「乗数を1箇所追加で掛ける」×2箇所（`makeProductionDecision`の通常候補ループとstickyな`activeGoalGoodId`再評価の両方）に限定し、複雑な勝者総取りループ自体の書き換えは避けた。

**副次的な修正**: Wineの`trade.durability`をPhase 4実装時の4から5（1-5スケールの最大値）へ引き上げた——Phase 4時点ではWineとRaisinsの`durability`が偶然どちらも4のままで、設計doc自身の「Wineは最も緩やか」という主張と矛盾していたため、本フェーズの前提として先に是正した。

**既存テストへの影響**: `production-generator.ts`には既存の専用単体テストファイルが存在せず（`seasonalPricing.integration.test.ts`/`shipbuildingInitialStock.test.ts`など間接的な統合テストのみ）、いずれもWine/Grapes/Raisinsを直接参照していないため回帰なし（実行して確認済み）。新規`viticultureAllocation.test.ts`（8件：初見シェアの即時確定・durability比例の収束速度差の直接検証・Burg間の独立性・追跡対象外Good/未記録シェアでの無効化・バイアス乗数の線形写像・クリア）を追加。economy拡張全体896件（888+8）が全てgreen、`tsc --noEmit`（テストファイル込みでも確認）/`lint`/`madge --circular`/`npm run build`もすべてクリーン。

これで`docs/plan/biome-goods-producer-ecosystem.md`§8のPhase 1〜5がすべて実装完了となった。残るPhase 6（Burg雇用構成への接続、§6）・Phase 7（不漁・豊漁の変動モデル、§5.2）は未着手のまま。
...
### 5.3 ブドウ栽培 → Grapes・干しブドウ・Wine（`viticulture.ts` + 加工工程）— ✅ 実装済み（Phase 4、§0 2026-08-06 Phase 4実装完了を参照）
...
### 5.4 牧畜 → liveAnimal家畜のキャリング容量（`husbandry.ts`）— ✅ 実装済み（Phase 3、§0 2026-08-06 Phase 3実装完了を参照）

§4.2で家畜ストックのキャリング容量が「牧畜労働力×牧草地/飼料配分」で決まるとしたことに対応する二次産業。ブドウ栽培・漁業と同じ§3.1の貪欲配分ループに参加する。**スコープの絞り込み（実装時に確定）**: 対象は開放牧草地で群れ管理される種——Cattle/Sheep/Goats/Horses/Camels（`isGrazedLivestockGood()`）——に限定した。Pig/Chicken/Cats/Dogsは屋敷回り・林間の小規模家畜で、専用牧草地や牧夫労働力を奪い合わないため、引き続きPhase 1/2の人口駆動フラットレートのままとする。

**牧羊犬による牧夫1人あたり許容頭数の乗数（2026-08-06調査、`docs/temp/herding-dogs.md`）**: シドニー大学の研究（Arnott et al., 2014、豪州800農場・4,000頭以上の使役犬を対象）は、訓練された牧羊犬を伴うハンドラー1人が最大2,000頭の羊、または500頭の牛を牧畜できると報告している——これは常勤の牧夫少なくとも1人分の労働力に相当する（ROI 5.2倍、生涯労働価値の中央値AU$40,000 対 生涯飼育・訓練費用中央値AU$7,763）。加えて、家畜を捕食者から守る「Livestock Guardian Dogs」は獣害による損失を68〜100%削減し、常時見張りに要する人的労力を大きく減らすとされる。**牧夫1人あたり基礎頭数（牧羊犬なしのベースライン）は史料の裏付けが薄いため、2026-08-06にユーザーへ確認し「推定値（羊200頭/牛60頭）」を採用する回答を得た**（§10.3で決定事項へ格上げ）。Sheep/Goatsの乗数10倍・Cattle/Horses/Camelsの乗数8倍は、Arnott研究の上限（羊2,000頭=200×10、牛500頭≒480=60×8）に整合するよう決めた。

**実装（`husbandry.ts`、新規）**:

- **牧草地面積**: `agriculturalLandUse.ts`の`calculateCultivableAreaHectares()`と同型のバイオームタグ別上限係数（grassland 0.85 / nomadic 0.75 / scrub 0.45 / dry 0.3 / mountain 0.25、タグ非該当は既定0.1）× 地形係数（同モジュールへ`calculateTerrainWorkableShare()`として抽出・共有）を、耕地・Burg市街地を差し引いた残地に掛けて算出する。
- **実際の牧草地利用面積**（`getPastureAreaUsedHectares`）は、この上限に牧夫の労働充足率（`getHusbandryWorkerFactor`、Wineの`getViticultureWorkerFactor`と同じ0..1充足率ゲート）を掛けたもの——人手不足のセルは牧草地上限まで到達しない。この利用面積は`getWildCarryingCapacity()`（faunaPopulation.ts）のwildHabitatArea計算からも差し引かれ、牧畜が使っていない分の土地は野生動物へ還元される。
- **家畜キャリング容量**（`getGrazedCarryingCapacity`）: 利用面積 × 種別の生態的許容密度（Cattle 0.5/Horses 0.4/Sheep 3/Goats 2.5/Camels 0.15、頭/ha、いずれも史料の一般的傾向からの概算・仮置き）。`faunaPopulation.ts`の`getDomesticatedCarryingCapacity()`は、グレイズド種にはこの値を、それ以外（Pig/Chicken/Cats/Dogs）には引き続きPhase 1/2の暫定プロキシ（フラットレート×24ヶ月）を使うよう分岐した。
- **必要牧夫数**（`calculateHusbandryDemand`）: セルのバイオームで生産可能なグレイズド種ごとの生（労働力ゲート前）需要量を、牧羊犬乗数を折り込んだ実効許容頭数で割って合算する。牧羊犬の充足度は、そのセルの`Dogs`個体群ストック頭数 ÷（牧羊犬なし換算の必要牧夫数 × 1牧夫あたり必要犬数3——仮置き）で決まる0〜1の連続値。農村労働配分器（`ruralOccupationAllocation.ts`）の§3.1貪欲配分に、漁業・ブドウ栽培（Wine）と並ぶ3つ目の候補として参加し、需要量に対する需要加重平均`good.value`で優先順位が決まる。
- production-utils.tsは、グレイズド種の`liveAnimal`継続生産レートに`getHusbandryWorkerFactor()`を掛けてから`drawDomesticatedFaunaOfftake()`へ渡す（Wineと同じ扱い）。この労働力ゲートは`options.ruralEcosystemDetail`に関わらず常時有効（§11.2どおりPhase 1相当の農村労働配分器は常時稼働）。

**`Dogs`Goodの新規登録（実装済み、前回ターン）**: `goods-generator.ts`へ`Dogs`（`tags: ["liveAnimal", "herding"]`、非食用、`unit: "head"`、Cats同型の低weight/bulk・高lossRiskの交易プロファイル、`Nomadic`/`Hunting`文化に高倍率）を追加済み。上記のとおり`husbandry.ts`の牧夫必要数計算から実際に参照されるようになった。番犬・闘犬など`Dogs`の亜種は、需要（軍事/治安/娯楽）や生産性への効き方が異なる別Goodとして、必要になった時点で追加を検討する（本フェーズでは追加しない）。
...
| 3 ✅ | 牧畜（`husbandry.ts`、§5.4）の面積・労働定数を確定し導入。家畜キャリング容量（§4.2）と接続。牧羊犬（`Dogs`Good）の頭数に応じた牧夫1人あたり許容頭数の乗数を配線した。実装: `husbandry.ts`（§0 2026-08-06 Phase 3実装完了を参照）。対象はグレイズド種（Cattle/Sheep/Goats/Horses/Camels）に限定、Pig/Chicken/Cats/Dogsは引き続きPhase 1/2の暫定プロキシ | Phase 1, 2 |
| 4 ✅ | `Grapes`・`Raisins`の新規Good追加、ブドウ収穫段階の労働力ゲート（§5.3の収穫段階、農村労働配分器に統合）、Wineのレシピを`{ Grapes, Barrels }`へ切替。加工段階（Grapes→Raisins/Wine）は既存`production-generator.ts`の`runWorkerLoop`＋`craft`セクターに新規レシピを乗せるだけ（2026-08-06訂正、新規機構は発明しない）。Fish→保存食（`Stockfish`/`Preserved food`）は既に実装済みのため本フェーズでの作業なし。実装: `viticulture.ts`（§0 2026-08-06 Phase 4実装完了を参照） | Phase 1 |
| 5 ✅ | 3分岐・保存食配分の需要連動＋`durability`比例の再配分速度（§9.4）。生食Grapesは残余として自動達成のため対象外、Wine/Raisins間の配分シェアをEMA的に平滑化。実装: `viticultureAllocation.ts`（§0 2026-08-06 Phase 5実装完了を参照） | Phase 4 |
...
2. **決定（実装済み）**: `Grapes`を独立した交易可能な新規Goodとして新設し、生食・`Raisins`（干しブドウ、保存食）・`Wine`の3方向に加工する（§5.3）。生食への配分は、WineにもRaisinsにも変換されなかった残余として構造上自動的に成立するため専用ロジック不要——Phase 5（実装済み、`viticultureAllocation.ts`）はWine/Raisins間の配分シェアにdurability比例の平滑化を追加した。
...
4. **決定（実装済み）**: 生食・`Raisins`・`Wine`の配分比率は需要連動の可変値とし、再配分速度を各商品の`good.trade.durability`に反比例させる（Wineは緩やか、生鮮Grapesは即応）。実装は`viticultureAllocation.ts`のWine/Raisins間EMA平滑化（レート`0.5/durability`）＋Wine/Raisinsの`makeProductionDecision`スコアへのバイアス乗数として実現し、生鮮Grapesの即応性は「未変換分がそのまま残る」構造から自動的に得られるため専用コードは不要だった。
...
7. **決定（§10.3、実装済み）**: 牧畜（`husbandry.ts`）の面積・労働定数は史料からの概算・類推を優先する。十分なデータが見つからない場合は仮置きせずPhase 3着手時に改めて相談する。**2026-08-06追記**: 牧羊犬**あり**時の牧夫1人あたり許容頭数の乗数はArnott et al. 2014（`docs/temp/herding-dogs.md`、§5.4）で裏付けが取れた。乗数を掛ける前の**ベースライン**頭数（牧羊犬なしの場合）はユーザーへ相談し、「推定値を使う（羊200頭/牛60頭）」の回答を得て確定した——牧羊犬乗数（Sheep/Goats×10、Cattle/Horses/Camels×8）はArnottの上限値（羊2,000頭・牛500頭）に整合するよう決めた。牧草地面積の土地適性係数（バイオームタグ別、grassland 0.85等）・種別生態密度（頭/ha、Cattle 0.5等）は史料の一般的傾向からの概算・仮置き。関連して`Dogs`Good（`liveAnimal`＋`herding`タグ）を新規登録済み、`husbandry.ts`から実際に参照される（§5.4）。
...
1. 害獣（獣害）のセル密度を表す指標が現状どこにも存在しない。§5.1の狩猟自給枠をこれで増減させる拡張は、その指標自体の設計をどこかのフェーズで別途行う必要がある（`threatCullHire.ts`/`cullPractice.ts`の脅威イベント頻度と関連付けるのが有力候補だが未検討）。**2026-08-07**: Wild（Game）がforest限定になっている現状の非現実性（サバンナ等の非森林バイオームが常に0）とあわせて`docs/plan/fauna-biome-realism.md`で調査・計画を開始した。同書§1.2はこの項目が指す`biomePredators.ts`/`threatCullHire.ts`系との統合を明示的にスコープ外とし、economy拡張側で新設するかどうかは選択肢として提示するに留めている。
...
3. ~~Phase 3で`husbandry.ts`が実装されたら、暫定プロキシを実際の牧草地/労働力ベースの値に差し替える必要がある——差し替え時、急激な在庫スケーリングが一度だけ発生しうる~~ → **解決（Phase 3実装済み、グレイズド種のみ）**: `getDomesticatedCarryingCapacity()`はCattle/Sheep/Goats/Horses/Camelsのみ`husbandry.ts`の牧草地/労働力ベースの値に切り替え、Pig/Chicken/Cats/Dogsは暫定プロキシのまま残した（§5.4のスコープ絞り込み）。想定していたダイオフの一度限りの急変は、実際に切り替え対象5種で起こりうる——既存セーブのグレイズド種`faunaStock`は旧プロキシ値ベースで成長済みの可能性があり、新しいキャリング容量（多くの場合、実面積に基づきより保守的）に対して`advanceCohortsOneYear`のダイオフ処理が次の年次更新で一度だけ働く。緩和処理（複数年かけた段階的収束等）は未実装——実際のプレイでの体感を見てから要否を判断する。
...
5. Phase 3の`husbandry.ts`は「1牧夫あたり必要犬数（`DOGS_PER_HERDER_FOR_FULL_COVERAGE`＝3）」を仮置きした——牧羊犬の充足度（0..1）をこの定数で決めており、Arnott研究はこの具体的な比率までは示していない（一般的な牧羊犬運用の「1人あたり2〜4頭」という記述からの類推）。校正が必要になった場合はこの定数を調整する。
6. Phase 4の`viticulture.ts`は、Grapesに参照できる既存`biomeOutputByTag`レートが存在しないため、「土地上限（`vineyardCeilingArea`）」とは別に「人口比例で実際に耕作を試みる面積（`VINEYARD_AREA_PER_POPULATION_POINT`＝0.5ha/人口点、仮置き）」という第2の上限を新設した——設計時点（§5.3）ではこの区別は明示されておらず、実装時に必要になった追加の設計判断（§0 2026-08-06 Phase 4実装完了参照）。この定数と`GRAPE_YIELD_PER_HECTARE_PER_MONTH`/`VINEYARD_LABOUR_DAYS_PER_HECTARE`は史料的裏付けのない仮置き（§9.3方針）。husbandry.tsの「牧夫1人あたり許容頭数」のような史料照会・ユーザー確認は行っていない——viticultureの労働定数は§10.3で名指しされた対象ではなく、`docs/temp/herding-dogs.md`のような具体的な調査資料も提供されていなかったため。
