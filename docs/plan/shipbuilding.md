...
* **技術ツリー**: 船級（Sloop → Galleon → Ship of the Line 相当）ごとに必要研究ポイントを定義。研究ポイントの蓄積速度は `getEffectiveSkill(ruler, "engineering")` で補正する。**大砲は前提にしない**: 世界観として銃火器は標準ルール外（弓矢・白兵戦が基本）であるため、船級ティアは「積載量・航洋性・乗員数」を軸にした輸送・遠洋航行寄りの木で設計し、`Gunpowder`/`Artillery` Good（既存）を用いた本格的な戦列艦（Ship of the Line）ルートは、オプション機能として別ゲート（`options`フラグ）の裏に隠す。
* **森林の回復**: Economy拡張側の管轄（上記参照）。Shipbuildingは伐採量の通知のみ行い、回復ロジック自体は持たない。
...
1. **Phase 0**: host時間経過エンジン（`timeEngine.ts` + `registerTimeTickHook` + `advanceTime` アクション）。UIトリガーのみ、フック先はまだ無い。
2. **Phase 1**: Shipbuilding拡張の骨格。Shipyard候補都市の判定とレイヤー表示のみ（シミュレーションなし）。
3. **Phase 2**: 伐採→丸太、Wood Good消費イベント連携。
4. **Phase 3**: 造船キュー・技術ツリー・tickフック接続。
5. **Phase 4**: Militaryへの艦隊強化イベント連携。
