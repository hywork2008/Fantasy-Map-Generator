...
| 森林の再生と生産 | Economy は伐採イベントを `forestStock.ts` に渡し、host の `simulation.cells.forestStock` を減らす。Wood のセル産出量、造船用伐採、森林回復はこの単一ストックを共有する。毎年、作付済みでない伐採跡だけが潜在森林容量へ向けて回復する。`pack.cells.biome` は変えない。 | Wood の在庫は後述の市場在庫に現れる。再生は在庫を直接増やさず、次の生産周期以降の供給を回復させる。 |
...
* `fmg:shipbuilding-log-harvested`（Phase 2） — `{ cellId, burgId, amount, deltaYears }`。Shipbuildingが伐採進行をEconomyへ通知し、`economy/generators/forestStock.ts` がそのセルの standing timber を減らす。
...
* **森林の回復（Phase 6で実装済み）**: `economy/generators/forestStock.ts` の `tickForestRegrowth(deltaYears)` が、`advanceTime()` のたびに作付済みでない伐採跡の `forestStock` を年2%で回復させる。**Shipbuilding拡張が無効化されていても回復は止まらない**（`isExtensionEnabled("economy")` のみをチェックし、Shipbuildingの状態は見ない）。Production再計算は既存のmicrotaskベースの `scheduleProductionRefresh()` を再利用し、伐採イベントと回復ティックのどちらが先に走っても1回の `advanceTime()` 呼び出しにつき`Production.produce()`は高々1回にまとめられる。
