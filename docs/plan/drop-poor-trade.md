...
ステップ1は実装済み。ステップ2（荷役・集積）の基本形は 2026-08 に導入済み（定期便カレンダーは未導入）。
...
| 新仕様2: 海路優先ルーティング | 実装済み | `TradeAnimation.findRoutePath` は海路単独探索を先行し、失敗時だけ陸海混合探索へフォールバックする。各辺は実経路距離・陸海速度・港での切替2日で評価する。 |
| ステップ2: 荷役・集積 | 部分実装 | `Caravan.state = "loading"`、同一 O/D マージ、target 55% / min 20% / maxWait 陸14・海10日、出航時に車両再選定・資産予約。定期出港カレンダーと export staging 倉庫は [merchant-logistics-warehouses.md](merchant-logistics-warehouses.md) 後続。 |
...
### 新仕様2 (実装済み): 海路優先ルーティング (All-Water Route First)
...
| **新仕様2: 海路優先ルーティング** | **小〜中**（Dijkstraの最大2回実行） | 海路単独探索が失敗した場合だけ、移動日数を重みとする陸海混合探索を行う。 | **実装済み** |
...
- 既存の輸送費価格計算は直線近似距離を維持する。一方、新仕様2の探索コストは実経路距離を陸海それぞれの速度で割り、切替ごとに2日を加える。
