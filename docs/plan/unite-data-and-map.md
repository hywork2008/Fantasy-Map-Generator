...
- [ ] extension module augmentation を削除し、extension caller を namespaced slice interface へ移す（Nobility の `State` augmentation と Economy の `Burg` augmentation は削除済み。Characters と Nobility の ruler / player conflict、Economy の burg production own caller は移行済み。Economy の `PackedGraph` fields と Shipbuilding は `pack` compatibility projection を使用中）。
