...
- [ ] extension module augmentation を削除し、extension caller を namespaced slice interface へ移す（Characters と Nobility の ruler / player conflict own caller は移行済み。Economy の burg production と Shipbuilding は `pack` compatibility projection を使用中）。
