...
> **後続計画**: 商船を有限隻で実ルート運用し、Vessel assets に現在地・次寄港地を持たせる設計は
> [vessel-itinerary-and-finite-trade-fleet.md](vessel-itinerary-and-finite-trade-fleet.md)。
> 本節の「平時はほぼ全艦が voyage」は暫定で、同計画 P1 では **商船は到着後 docked/idle**、
> 抽象 voyage 収入は国家艦の訓練（patrol）に限定する方向へ更新する。
