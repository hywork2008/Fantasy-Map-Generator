...
**2026-08-07 Phase J（Cheeseの産地とCattle/Sheep/Goatsの産地の不一致を修正）**: ユーザーから「チーズを産出する地域が牛・羊・山羊のいる地域と合致していない」との報告。原因調査の結果、Cheeseは通常のburgクラフト・レシピ（`{ Cattle: 0.5, Salt: 0.25 }`等、`production-generator.ts`の`Markets.buy()`経由）として実装されており、この購入は**burgが属する地域市場の交易可能な在庫プール**から行われる——市場は州・地域単位でCattle/Sheep/Goatsを含む全商品在庫をプールしているため、家畜が実際に育っていない地域のburgでも、隊商等で市場に牛が流入していれば普通にCheeseが作れてしまっていた。GrapesやGame（狩猟）は同じ問題を避けるため、通常のレシピ機構を使わず、そのセルの現地生産量から直接計算する専用の仕組み（`getGrapeHarvestOutput`/`getHuntingGameOutput`、`production-utils.ts`の`getRuralProductionContributions()`内で特別扱い）を持っている。

**ユーザー確認（AskUserQuestion）**: 修正方式は「A: 現地直接計算に切り替え（Grapes/Gameと同型、新規`Milk` Good・取引不可フラグは導入しない）」を選択——「B: 新規Milk Good + 取引不可フラグを新設」（前述のPhase F案に忠実、影響範囲が広い）は不採用。

**実装**: (1) `goods-generator.ts`からCheeseの`recipes`フィールドを削除（Salt/Vinegarによる保存加工ステップは、この簡略化により意図的に失われる——開示済みのトレードオフ）。(2) 新規`dairy.ts`を追加——`getCheeseOutput(cellId)`が、faunaPopulation.ts の個体群ストックテーブルから**そのセル自身**のCattle/Sheep/Goats実頭数を読み、種別ごとの`CHEESE_YIELD_PER_HEAD_PER_MONTH`（Cattle 0.05 > Goats 0.02 > Sheep 0.015、史料的裏付けのない相対順序プレースホルダー、§9.3方針）を掛けて合算し、`getHusbandryWorkerFactor(cellId)`（同じ牧夫が搾乳も担うという想定、husbandry.ts/viticulture.tsの労働力ゲート前例を踏襲）で最終ゲートする。狩猟・グレイズド種のoff-takeと異なり搾乳は個体群を間引かないため、プレビュー版と実行版の分岐は不要（関数自体が既に副作用なしの読み取り専用）。(3) `production-utils.ts`の`getRuralProductionContributions()`にGrapesと同型の特別扱いブロックを追加し、Cheeseをrural production経路に接続。Horses/Camelsは元のCheeseレシピに含まれていなかったため、乳製品種から意図的に除外（Cattle/Sheep/Goatsのみ）。

**Playwright実地検証（Phase J時点）**: 生成直後のマップで、現地の乳製品種頭数から予測されるCheese月産合計（約178,321、716セルが正の乳製品ポテンシャルを持ち、その全セルでhusbandry労働力充足率が既に1.0）に対し、実際の市場在庫合計は約101,701——同じ桁でプラスに相関しており、パイプラインが意図通り機能していることを確認した（両者が完全一致しないのは、市場間交易・需要消費・生成サイクルの途中スナップショットによる差異で、想定内）。「Failed to acquire ... for production of Cheese」エラー（旧レシピ機構の失敗ログ）はコンソールから完全に消えたことも確認。新規`dairy.test.ts`6件（現地に乳製品種が無ければ0／労働力ゼロなら0／頭数と労働力充足率に比例／Cattle・Sheep・Goatsの合算／Horses・Camelsは寄与しない／**別セルの頭数は一切参照しない**——地理的不一致の直接的な反証テスト）追加、全件green。経済拡張含む全体2209件（既存の無関係な6件の失敗を除き）green、`tsc --noEmit`/`lint`/`madge --circular`もクリーン。

**2026-08-07 Phase K（Milk中間財を新設し、Cheeseをレシピベースに戻して塩/酢の物流と雇用を復活）**: ユーザーから「ミルクからチーズを作る際に塩か酢を消費するレシピは残して、塩や酢の生産地からミルクのある農村への物流が発生し経済が回るようにしたい。チーズ生産者の雇用、場合によってはギルドとスキルまで」との要望。Phase Jの直接計算方式（Cheeseを`recipes`もMarket購入も一切介さずに算出）は地理的不一致を完全に解消した反面、`production-generator.ts`の通常のburgクラフト・レシピ経路から完全に外れるため、Salt/Vinegar需要も、`productiveGoods`ループが担うクラフト従事者雇用・ギルドボーナスも失っていた——このトレードオフをPhase Jの時点で明示的に開示済み。

**実装**: (1) `dairy.ts`の`getCheeseOutput()`を`getMilkOutput()`に改称——ロジック自体は不変（そのセル自身のCattle/Sheep/Goats頭数×種別レート×`getHusbandryWorkerFactor`）だが、算出対象をCheeseから新設の`Milk` Goodに変更。レートはCheese換算値の3倍（Cattle 0.15/Goats 0.06/Sheep 0.045）に再スケール——後述のレシピ比率`Milk:3`を通した後のCheese産出量が、Phase Jの直接計算とおおむね同じ桁に収まるようにするため。(2) `goods-generator.ts`にMilkを新設（`tags: ["food", "freshFood"]`——Grapesと同じ「生鮮食品」タグで、`tradeOpportunityEstimator.ts`の既存の日数上限機構（無冷蔵想定で高温気候では1〜2日）が長距離隊商交易を経済的に成立しづらくする、新規の「取引不可フラグ」を発明せずに済む）。Milkは`production-utils.ts`の`getRuralProductionContributions()`にGrapesと同型で接続（Cheeseから移設）。(3) Cheeseの`recipes`を`{ Milk: 3, Salt: 0.25 }`/`{ Milk: 3, Vinegar: 0.25 }`として復元——通常のburgクラフト・レシピ経路（`production-generator.ts`）に戻ったことで、クラフト従事者雇用（`craftWorkersByBurg`）は自動的に復活する。**ギルド/スキルは今回未対応**——`guildKnowledgeTypes.ts`の`CRAFT_DOMAIN_BY_GOOD_NAME`は「食品・嗜好品系のレシピ財は元々ギルド適用対象外」という既存方針を明記しており（Bread等、他の食品レシピ財も同様に無指定）、`CRAFT_KNOWLEDGE_DOMAINS`にも食品/酪農ドメインは存在しない——新設は今回のスコープを超える追加の意思決定が要るため見送り、Cheeseは他の食品レシピ財と同じく「雇用はするがギルドボーナス無し」の扱いのまま。

**注記（地理的精度のトレードオフ）**: この変更は「セル単位の完全一致」だったPhase Jの精度を、「同じ地域市場（Market territory）単位」まで意図的に緩めている——Marketは複数セルの現地生産を1つのプールに集約するアーキテクチャ上、Milk（Good化した以上）もこのプールに乗り、同じ市場圏内の別セルのburgからも購入可能になる。これは狙った振る舞いで、Salt/Vinegar生産地からミルク産地への物流という経済循環を実現するには、Cheeseが再び「地域市場から購入するレシピ財」である必要があったため。

**Playwright実地検証**: MilkのGood定義・Cheeseのレシピ（`{Milk:3, Salt:0.25}`/`{Milk:3, Vinegar:0.25}`、名前→数値id解決も含め）が実際のマップで正しく登録されることを確認。生成直後、Milkは5市場全てで非ゼロ蓄積（2年後約103万まで増加）——現地限定生産が実際に機能していることを確認。一方Cheeseの市場在庫は生成直後・2年後・7年後のいずれも0のままだった——原因を追ったところ、このテストマップではSalt（生成直後合計5、7年後も39.5）とVinegar（生成直後合計2、7年後も4）自体が地図全体で慢性的に希少で、Cheeseの生産候補が`planGoodAction`の`projectedGain`フィルタで採算ライン未満と判定され続けていたためと判明（コンソールに「Failed to acquire...for production of Cheese」ログが一切出ていないことから、購入試行自体に到達する前の候補選定段階で弾かれていることを確認——エラーではなく「まだ経済的に見合わない」という正常系の待機状態）。Salt/Vinegar自体の希少性は、Saltの`demandCoverage: {utilities:1}`（一般消費との競合）とVinegarのWine経由の多段クラフト連鎖という、今回のdairy固有スコープとは別の既存要因に起因する可能性が高く、今回は未調査——Cheese-Milk配線自体の正しさ（レシピ解決・現地限定生産）は機構的に確認できたが、「実際に塩/酢が届いてチーズが焼き上がる」までの経済的成立は、このテストマップでは実証できなかった。新規`dairy.test.ts`をMilk向けに書き換え（6件、全件green）。経済拡張含む全体2209件（既存の無関係な6件の失敗を除き）green、`tsc --noEmit`/`lint`/`madge --circular`もクリーン。i18n（en/ja）にMilkの翻訳エントリを追加。

**2026-08-07 Phase L（Saltのbiome生産バグ修正 + 「食品系クラフト財が地図全体で一切作られない」というより深い既存問題を発見）**: ユーザーから`docs/temp/0807-vinegar.md`（史実の塩・酢の需給構造に関する調査メモ——酢は主にワイン/蜂蜜等の劣化品からの副産物として地産地消され、長距離交易は高級品のみ；塩は小麦の0.5〜10倍超という広い価格変動幅を持ち、産地からの距離・輸送費・税が主要因）を踏まえ、「ミルクをゲーム開始時点からチーズにしておくため、塩と酢の製造と流通を整えたい」との要望。

**発見1（Salt、確定バグ、修正済み）**: Saltの`biomeOutput: { 1: 0.1, 2: 0.1 }`（hotDesert/coldDeserの数値コードのみ）が、この同じGoodの`distribution`（cosmeticなマップアイコン配置式）が示す範囲——tidal flat/塩性海岸(`shore(1) && type("salt","dry")`)/砂漠(70%)/湿地(10%)——よりずっと狭く、今セッション既に発見・修正したCattle/Elephants/Camelsと全く同じ「distributionはtag/地形で広く言っているのに、実際の経済生産(biomeOutput)は数値コードで狭く縛られている」というバグパターンだった。`biomeOutputByTag: { desert: 0.1, wetland: 0.015 }`へ移行（wetlandは"distribution"の10%ロール相当として砂漠の約1/7の低いレートに設定、湿地/沿岸塩田が砂漠塩に対し副次的な供給源という史実の相対順序を反映）。**未解決の限界**: `shore(1)`/`coastalHabitat("tidalFlat")`は地形タグではなくセルの海岸隣接という地理的性質で、`resolveBiomeOutputRate()`はセル隣接情報を一切扱えないため、「海からの距離」という調査メモが指摘する主要な価格決定要因そのものは今回も表現できていない——プレースホルダーとして残る。

**実地検証**: 生成直後のSalt市場在庫合計が約5→約61に、2年後は約10→約122に上昇（10倍以上）——修正の効果を確認。

**発見2（未修正、より深い既存の系統的問題）**: Salt増産後もCheeseは全市場で在庫0のまま、かつ全市場でMilk（数万〜数十万）とSalt（1.4〜52.7）が共存しているにもかかわらずCheeseは全く作られていなかった——材料不足ではなく、burgのクラフト生産選択ロジック（`production-generator.ts`の`planGoodAction`/`chosenGoal`）がCheeseを一切選ばないことが原因と判明。さらに調査すると、**Cheeseだけでなく、Wine・Vinegar・Bread・Preserved food——「food」カテゴリのクラフト・レシピ財ほぼ全て——が同じテストマップで軒並み在庫0**だった一方、Boots/Garments/Cloth（革・繊維系クラフト財）は普通に生産されていた（それぞれ25.78/15.98/4.28の在庫）。これは今回のMilk/Cheese固有の問題ではなく、**foodカテゴリのクラフト財全般がburgの生産選択で恒常的に不利になっている、セッション開始前から存在する系統的な問題**である可能性が高い——おそらく`getDemandEffect()`のfoodカテゴリ需要充足度計算（ルーラルな穀物生産だけで既にfood需要が飽和し、クラフト労働力にとってfoodカテゴリ財の限界価値が常に他カテゴリ財に負ける）が原因の仮説だが、未検証。スコープが今回のdairy固有の依頼を大きく超えるため、着手するかはユーザー確認が必要。

**Playwright実地検証**: 上記の通り、Salt修正は確認済み。food系クラフト財の生産停止は`tsc`/`lint`/`madge`済みの範囲外（コード変更なし、観察のみ）。新規/変更テストなし（Salt変更はデータ値のみで既存テストに影響なし）、全体テスト・lint・madge実行結果はPhase K時点から不変。

**2026-08-07 Phase M（foodカテゴリのクラフト財がburgの生産選択で恒常的に不利になる系統的問題を特定・修正）**: Phase Lで発見した「food系クラフト財が地図全体で一切作られない」問題の調査を継続。

**根本原因の特定**: `production-generator.ts`の`makeProductionDecision()`は、burg単位の需要カテゴリ（`food`/`utilities`/`construction`/`military`/`hunting`/`luxury`、この優先順の固定リスト`DEMAND_PRIORITY`）を先頭から走査し、**最初に見つかった「未充足」カテゴリだけ**を今回の"focus"にする（`getDemandFocus()`）。そのfocusカテゴリを`demandCoverage`でカバーするGoodだけが`multiplier = 1 + coverageWeight × shortage`という需要ブーストを受け取る（`getDemandEffect()`）。Cheese/Wine/Vinegar/Bread/Preserved food等の`demandCoverage`は`{food: ...}`のみのため、focusが`food`以外の時はブースト倍率が常に1（無補正）になる。一方、ルーラルなGrain生産（労働力ゲート無し、人口比例のフラットレート）だけで`demandTargets.food`（人口×`DEMAND_TARGET_FACTORS.food`=0.2）はほぼ常に飽和しており、`food`のshortageはほぼゼロ——`getDemandFocus()`のループは`food`を素通りして`utilities`等に飛ぶ。結果、food専用クラフト財は原材料費を払っても需要ブーストを一切受け取れないまま、常にブーストを受け続ける他カテゴリ財と生の利益率だけで戦わされ続け、恒常的に敗北する。ユーザーの見立て（「儲けの小さい商品を運ばないようにする交易側のフィルタでは」）とは異なるメカニズムだったが、「利益率の低いfood財が構造的に弾かれる」という現象自体は正しく言い当てていた。

**ユーザー確認**: 選択肢A（長距離交易財の需要を絞る）はボトルネックの実態と一致しないため不採用。選択肢B（クラフト食品に最低保証生産枠を新設）・C（food需要ターゲットを量と質に分割）を提示したところ、ユーザーからカスタム回答——「地域での食料生産、生産したもののうち腐りやすい食料品などの保存食への加工にまず生産力・労働力を使い、それらが終わったらその他の生産に労働力が使えるようにしてほしい」——という、優先順位付け（B案の精神に近いが、対象を「腐りやすい食料品の保存食加工」に絞った、より具体的な指定）を受けた。

**実装**: `production-generator.ts`の`ProductionIndex`に`preservationGoods`を新設——`productiveGoods`のうち`tags`に`"food"`を含み、かつレシピの原材料に1つでも`freshFood`タグ財（Fish/Game/Milk/Shellfish/Grapes）を含むGoodを動的に抽出する（ハードコードした名前リストではなく、既存のタグ体系から機械的に導出——現状はWine/Preserved food/Cheese/Stockfish/Raisinsの5種が該当）。`runWorkerLoop()`を2段階化: **フェーズ1**は`preservationGoods`のみを候補にした生産選択ループ（`makeProductionDecision()`に新設の`candidateGoods`引数で候補集合を差し替え可能にした）を、原材料が入手可能かつ`projectedGain > 0`（既存のロス防止ガード、無改修）である限り繰り返し実行——需要ブーストの有無に関わらず、常に他カテゴリより先に労働力を使う。原材料（Milk/Grapes/Fish等）が尽きるか採算が合わなくなった時点でフェーズ1は自然終了。**フェーズ2**は残り労働力で従来通りの全`productiveGoods`を対象にした利益ランキング・ループ（無改修）。両フェーズ間で`state.activeGoalGoodId`をリセットし、フェーズ1の選好がフェーズ2のランキングに漏れ込まないようにした。人為的な労働力上限は設けていない（原材料の入手可能量が自然な上限として機能する——ユーザーの「それらが終わったら」という指定通り）。

**Playwright実地検証**: 修正前は生成7年後もCheese/Vinegar/Wine/Preserved food全て在庫0だったのに対し、修正後は**生成直後（tickCount=0、Advance Time前）**の時点で既にCheese 24.05・Vinegar 5.43・Wine 5・Preserved food 167.17・Stockfish 3.8・Raisins 32.76が市場在庫として存在することを確認——「ゲーム開始時点から」という当初の要望を文字通り満たした。5年後にはCheese 97.88・Wine 142.1・Preserved food 374.44まで成長。並行してBoots/Garments/Clothも1年目は0だったが5年後には6.13/40.7/9.32まで回復しており、フェーズ1がフェーズ2の労働力を恒久的に奪っているわけではないことも確認した（原材料が尽きればフェーズ1は自然に手を引く）。**既知の残存ギャップ**: `freshFood`タグを持たない主食由来のfoodクラフト財（例: GrainベースのBread）は今回のフィルタ条件に該当せず、`getDemandFocus`の構造的な問題からは引き続き影響を受ける——ユーザーの依頼が「腐りやすい食料品の保存食化」に明示的に絞られていたため、意図的にスコープ外とした。Leather/Tools/Arms等の一部非food財も本テストマップでは低調だったが、Cattle等原材料自体は入手可能で、food系とは無関係な別要因（ランプアップ中、または別の構造的原因）の可能性が高く、今回は未調査。テスト: `production-generator.ts`に専用の既存ユニットテストが無く、大規模な結合的セットアップが必要なクラスのため、新規ユニットテストは追加せず上記のPlaywright実地検証で代替（既存テストスイート2209件は無関係な6件を除き全件green、`tsc --noEmit`/`lint`/`madge --circular`もクリーン）。

**2026-08-07 Phase N（Milk余剰が過大——供給レートの再校正 + 新規消費財追加）**: ユーザーから「ミルクの残余が多すぎる」との報告と、`docs/temp/0807-cheese.md`（現実のチーズ作りにおける凝固剤——レンネット/植物性凝乳酵素/酸性果汁/木灰の解説と、レシピ改定案）を踏まえ、新商品追加も視野に入れた対応の依頼。加えて「家畜の排出するミルクの量が多すぎないかもチェックして下さい」との明示的な較正確認の依頼。

**実態の確認**: Phase Mの検証で使用したテストマップでは、5年後にMilk市場在庫合計が106万超に達していた一方、消費側（Cheese）は5年後でも97.88（Milk換算で約294）——供給の0.03%未満しか消費されていなかった。`STOCKING_DENSITY_PER_HECTARE`（Sheep 3頭/ha等、husbandry.ts既存）× `MILK_YIELD_PER_HEAD_PER_MONTH`（Phase K設定値）を掛け合わせると0.075〜0.15 Milk/ha/月という、種間で内部整合的だが、数千ha規模の牧草地上限と掛け合わさると絶対値が過大な水準だったと判明——**供給レート自体が主因**で、需要側だけの追加では焼け石に水と判断した。

**修正1（供給レートの再校正）**: `dairy.ts`の`MILK_YIELD_PER_HEAD_PER_MONTH`を全種一律で約15分の1に引き下げ（Cattle 0.15→0.01／Goats 0.06→0.004／Sheep 0.045→0.003、種間の相対順序は維持）。数式的な厳密な導出ではなく、Playwrightでの複数年実地検証によって「なお増加はするが桁が現実的な範囲」に収まることを確認した第一次経験値（§9.3方針通り、要すれば再調整）。

**修正2（新規消費財、`docs/temp/0807-cheese.md`ベース）**: 同ドキュメントが提案する4方向（レンネット/植物性凝乳酵素/酸性果汁/木灰）のうち、**レンネットと木灰の2つを実装**した——理由は、この2つだけが「既に地図上で豊富に流通している既存素材（家畜頭数・Wood）からの安価な副産物」として実装でき、植物性凝乳酵素（Thistle/Fig/Nettle）や酸性果汁（Lemon/Lime）のように新規のバイオーム産出ルート丸ごとの新設を要さないため——Milk余剰という当面の課題への対応として費用対効果が高いと判断した。

- **Rennet**（新規Good）: `{ Cattle: 0.03 }`/`{ Sheep: 0.03 }`/`{ Goats: 0.03 }`という小規模な家畜頭数副産物レシピ（乳牛群である必要はなく、屠殺される家畜全般からの副産物という想定）。
- **Ash**（新規Good）: `{ Wood: 1 }`——Woodをそのまま混ぜるのではなく「燃やした後の灰」という体裁（ドキュメントの指摘通り）。ほぼ無尽蔵に近いWoodを原料にできるため、Salt/Vinegar/Rennetが局所的に不足しても代替できる、ほぼ普遍的に入手可能な凝固剤ルートになる。
- **Cheeseのレシピを4種に拡張**: `{Milk:3, Salt:0.25}` / `{Milk:3, Vinegar:0.25}` / `{Milk:3, Rennet:0.1}` / `{Milk:3, Ash:0.15}`——単一の希少な凝固剤への依存を解消し、Milk余剰を奪い合う独立した4本の原材料経路を持たせた。
- **Smoked Cheese**（新規Good）: `{ Cheese: 1, Wood: 0.5 }`。ドキュメントが明示的に区別する「凝固」とは別工程（燻製による保存性向上）として、Cheese自体をさらに消費する2次的なMilk消費経路兼、高価値の輸出向け財として追加。

Rennet/Ash/Smoked CheeseはいずれもPhase Mの`preservationGoods`優先パス判定（`food`タグ＋レシピにfreshFood原材料を含む）には該当しない——Rennet/Ashはfoodタグを持たず、Smoked Cheeseのレシピ原材料（Cheese/Wood）はどちらもfreshFoodではないため——通常のフェーズ2ランキングで生産される。Cheese自体は新レシピ4種のいずれもMilk（freshFood）を含むため、優先パス対象のままである。i18n（en/ja）に3財の翻訳エントリを追加。

**Playwright実地検証**: 生成直後（tickCount=0）でMilk市場在庫合計は約9,666（同条件の旧レートでは約19万〜27万——約20〜30倍の圧縮）。Cheese 53.23・Rennet 7.38・Ash 3.84が既にこの時点で生産されていることを確認。5年後、Milkは58,632（旧レートでの同期間実績は100万超——約18倍の圧縮）、Cheese 144.78・Smoked Cheese 4・Wine 82.03・Preserved food 223.85・Boots 40.73・Garments 79.11と、food/非food問わず健全に成長していることを確認。Milkの蓄積速度は依然として消費速度を上回っており（5年で在庫は約6倍に増加）完全な均衡には至っていないが、「実質無限の死蔵在庫」だった状態から「なお余剰はあるが妥当な桁」まで改善したと判断する——さらに絞る場合は追加のレート引き下げ、または個体群側の`STOCKING_DENSITY_PER_HECTARE`自体の見直しが次の一手になる。既存テストスイート（`dairy.test.ts`の期待値を新レートに合わせて更新）を含め全件green、`tsc --noEmit`/`lint`/`madge --circular`もクリーン。**未実装（今後の選択肢として記録）**: 植物性凝乳酵素（Thistle/Fig/Nettle、野草採集ルート）と酸性果汁（Lemon/Lime、地中海圏限定バイオーム）は、新規バイオーム産出財の追加を要するため今回は見送った——§6に追記。
...
- 2026-08-07、Milk/Cheeseが実際の人口の栄養需要をどれだけ満たすかの監査は`docs/plan/food-nutrition-audit.md`に分離した（Grain専用の食料台帳システムまで遡る調査になり、fauna/biomeの範囲を超えたため）——Grainの栄養充足率（カロリー約89%・タンパク質約131%）は妥当、Cheese/Milkは地図全体スケールでは無視できる規模（現時点では約0.005%）という結論。
- Phase Nで見送った`docs/temp/0807-cheese.md`の残り2方向——植物性凝乳酵素（Thistle/Fig/Nettle、野草採集ルート、動物を殺さず序盤/資源乏しい地域向け）と酸性果汁（Lemon/Lime、地中海圏限定バイオーム）——はどちらも新規のバイオーム産出財（新規`biomeOutputByTag`ルート）の追加を要する。実装する場合、Thistleはgrassland/scrub系タグ、Lemon/Limeは温暖・地中海性気候限定（`Mediterranean`バイオーム地域プロファイルとの連携も検討余地）といった、既存のCamels/Elephants等と同様のタグベース設計が妥当と思われるが、具体的なレート・対象バイオームは未検討。
