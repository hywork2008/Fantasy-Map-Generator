...
「標準」プリセットの人口/価格/在庫成長率は実測値に更新済み。**ただし国庫成長率は、実測値（約-42%/yr、
5シードで再現性あり——詳細は§5.3）が当初の見積もり（+1.5%/yr）と符号・桁とも大きく異なったため、
ユーザー判断により未確定のまま保留した——採用する前に経済バランス側の原因調査を別途依頼することになった
（2026-09-05、§5.3.3・§10・§11参照）。**
...
| **標準 (Steady)** | **+0.5%**（実測+0.49%） | **0.0%**（実測-0.11%） | **+8.5%**（実測+8.53%） | ⚠**保留（実測-42.47%だが未採用——§5.3.3）** |
...
**次のアクション（本書のスコープ外、別途調査が必要）**: `docs/plan/state-treasury-department-budget.md`や
`docs/plan/economy-coupling-audit.md`（既存の結合度監査、特に財政まわりの項目）を持つ調査に、「現行デフォルト
設定で国家財政が20年時点でも年-40%超のペースで先細りし続ける」という本Phase 0の実測事実を持ち込み、
年次投資系システム（`AgTechInvestment`/`IndustrialTechInvestment`等の`settleAnnual()`群、
[index.tsx:2994以降](../../src/extensions/economy/index.tsx)）やGuild/軍事維持費が税収を上回るペースで
`state.treasury`を消費している可能性（未検証、§5.3.3冒頭）を含めて、意図した難易度カーブなのか見落とされて
いた不具合なのかを判断してもらう。その結果が出るまで、本書「標準」プリセットの国庫成長率は確定させない
（§10・§11）。
...
- **（Phase 0で新規発見、未解決——次のアクション参照）** 現行デフォルトバランスで国家財政がシミュレーション
  20年目時点でも年-40%超のペースで先細りし続ける（§5.3.3）のは意図された難易度カーブか、見落とされていた
  既存の不具合か。本設計のスコープ外だが、経済バランスを見ているチーム/ドキュメント
  （`docs/plan/state-treasury-department-budget.md`, `docs/plan/economy-coupling-audit.md`）に別途持ち込む
  ——2026-09-05、ユーザー判断によりこの調査結果が出るまで「標準」プリセットの国庫成長率は確定させないことに
  決定した（本書§5.2/§5.3.3の保留マーク、および直後の「次のアクション」参照）。
...
2. **（ユーザー決定、2026-09-05）国庫の恒常的赤字（§5.3.3）は、Fast-Forwardのプリセットに採用するかどうかを
   決める前に、経済バランス側の原因調査を別途依頼することになった。** 本書はこの調査結果を待つ。調査自体は
   本書のスコープ外——`docs/plan/state-treasury-department-budget.md`/`docs/plan/economy-coupling-audit.md`
   の文脈に持ち込むか、新規の調査依頼として起票するかは未定。
