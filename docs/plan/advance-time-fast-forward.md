...
2. ~~国庫の恒常的赤字（§5.3.3）は、Fast-Forwardのプリセットに採用するかどうかを決める前に、経済バランス側の
   原因調査を別途依頼することになった。~~ **調査完了（2026-09-05）** ——新規起票した
   ...
   はそれ自体年-74程度で安定しており問題ない。修正方針（A: renewal debitを減額 / B: 州単位の集約予算上限を
   新設 / C: 意図的な難易度カーブとして現状維持）は未決定——本書の「標準」プリセット国庫成長率は、この方針が
   決まるまで引き続き⚠保留のままとする。
