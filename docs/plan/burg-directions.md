...
| 船を避ける / 海路優先 | なし | **「Avoid sea travel」「Prefer sea travel」オプション**。「船を避ける」ON で陸路のみ探索（陸路が無ければ `seaRequiredDespiteAvoid` で海路にフォールバック）。「可能な限り海路を使う」ON で陸路移動コストにペナルティを課して海路を最大活用（海路が無ければ `preferSeaNoEffect` で陸路のみ表示） |
...
「可能な限り海路を使う」ON 時は陸路コスト関数に1000倍のペナルティを掛けて探索し、陸路移動区間を極小化して
接続可能な海路を最大利用する。確定経路に海路が含まれない（陸路のみ）の場合は `preferSeaNoEffect: true` を
返してその旨を通知する。所要日数は探索後に実コストを集計して計算するため、探索重みによる表示歪みは生じない。
