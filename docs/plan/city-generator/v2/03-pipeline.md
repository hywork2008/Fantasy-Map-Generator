...
| ステージ | 入力 | 出力 | 主な不変条件 |
|---------|------|------|-------------|
| S0 パラメータ | `CityParams`（`site?` 含む、[13-fmg-site-input.md](13-fmg-site-input.md)） | 正規化済み params、ステージ別 RNG | 同一 params → 同一出力（以降全ステージ） |
| S1 地形 | params, rng("terrain"), site? | 高度場・傾斜場・川ポリライン・水域・橋候補点 | 川は単調に下る。高度場は連続。site モードでは descriptor の川・水域・マクロ標高に整合（fit メトリクス F1/F4/F5） |
...
5. **FMG site モードは入力の差し替えであって分岐ではない**: `CityParams.site` があるときは S1 の地形・川と S2 の門アンカー・S3 の街道端点を descriptor から取る（[13-fmg-site-input.md](13-fmg-site-input.md) §3）。S4 以降のアルゴリズムに site 有無の分岐を持ち込まない。
