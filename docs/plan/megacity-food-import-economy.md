...
**決定（改訂）**: 食料在庫は二層に分ける。農村セルは集約された私的在庫`ruralHouseholdFoodStock[cell]`を持ち、住民一年分の主食を通常消費用に保存する。収穫はまずこの目標を補充し、残余だけが Market 単位の共通在庫`foodStock`へ卸される。農村消費は私的在庫を先に引き、尽きたときだけ Market 在庫・輸入へフォールバックする。これにより自家消費を Market の生産・消費として二重決済せず、凶作時の救済経路は維持する。

Market 在庫は都市・宿泊者が通常購入する主食と取引可能余剰を所有する。在庫上限はこの Market 購入需要の年間量の**9か月分**（`annualMarketDemand × 0.75`）とする。地図生成時および既存セーブで economy を初めて有効化する時は、Market 在庫を年間 Market 需要の**6か月分**（`annualMarketDemand × 0.5`）、農村私的在庫を一年分で一度だけ初期化する。前者は都市の供給連続性、後者は農村の通年自給を表す。
