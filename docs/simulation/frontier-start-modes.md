...
The **capital is always the first realm cell**, and it sits on a **river** whenever one exists on a large enough landmass. Remaining starting-realm cells grow as adjacent land from that capital (`collectStartingRealmCells`), so a 2-cell, 10-cell, or 30-cell start is on water without a size-specific rule. Among rivers, a **coastal river-mouth** scores higher (true ocean harbor is required when the start is seaborne). **Continents (`feature.group === "continent"`) are filled first.** A large mainland with room for several homelands is not skipped so that each island can take one state. Several states on the same landmass are spaced along that coast (about 8 land-coast hops when dispersed, relaxing if the shore is short) instead of being shoved to opposite map corners. Islands are used only after every eligible continent site is taken. Land origin is not snapped to a riverless harbor.
