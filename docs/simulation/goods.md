...
- `agriculturalLandUse.ts` は、適する作物を穀物 65%・豆類 20%・根菜類 15% を目標に、セルの作付面積へ配分する。この組合せは三圃式の通常状態なので、初期収量に連作障害ペナルティはない。
- 豆類を育てられず穀物だけに偏る場合は、年次更新時に `soilFertility` が低下し、以後の収量を下げる。豆類の作付は緩やかに回復させる。
