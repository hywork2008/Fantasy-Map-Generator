...
- 後期技術: `waterLifting` は給水側を強化し、`sanitaryEngineering` は汚水輸出を抑え分流を解禁する（歴史時代の天井あり）。
- 異なる State 間の上下流汚染は state treasury 補償と未払い時の `state.alert` 上昇に接続する。
- Province / State の `sanitation` は、所属 Burg の平均でロールアップする。

治安（`security`）と倉庫衛生は、この段階ではまだ自動変動しない（汚染外交の `alert` は例外的なソフト効果）。
...
- 疫病イベント本体と人口死亡への直結、フル技術グラフ段階（locked/known/…）は未実装。
