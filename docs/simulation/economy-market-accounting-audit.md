...
最終確認: 2026-07-30（Food Ledger v2 マイルストーン1の実装後に更新）

## 結論

現行Economyの一般Goods側Marketは、物資在庫と価格を管理する主体であり、独立した貨幣残高・貸借対照表を持たない。`BurgMarketLedger.revenue`は今も売上を説明・表示するための導出データであって、商人が実際に支払える運転資金ではない。

一方、Food Ledger（stapleFood/Grain限定）では`marketTreasury.balance`・`ruralGrainPayable`が実装済みであり、これはMarketが実際に持つ運転資金・負債である。農村からのGrain仕入れは`balance`から支払われ、不足分は`ruralGrainPayable`として蓄積し、都市小売収入から優先返済される（[foodProduction.ts](../../src/extensions/economy/generators/foodProduction.ts)・[foodLedgerConsumption.ts](../../src/extensions/economy/generators/foodLedgerConsumption.ts)）。この局所決済は、当初の方針どおりFood Ledgerの農村⇄Market間だけに限定されており、一般Goods取引（`Markets.sell()`/`buy()`/`runGlobalTrade()`）や市場間のFood Shipment輸送はまだこの決済に接続していない。既存の一般Goods取引を直ちに完全な複式的決済へ置き換える計画は変わらず採らない。一般Goodsの貨幣会計を同時に変更すると、Burg生産、消費、税、隊商、商人ledger、State treasuryを一括で再設計することになり、食料不足・人口移動の導入範囲を大きく超える。
...
1. 「Marketが地域の作物を吸い取る」こと自体は、現行では物量上すでに起きている。stapleFood（Grain）については農村セルへの対価支払いが`marketTreasury.balance`/`ruralGrainPayable`で実装済みだが、それ以外の一般Goodsへの対価支払いは依然として存在しない。
...
3. 現行システムは、Burgの販売時に貨幣を増やし、購入時に貨幣を減らしてもMarketへ移さない。このため、一般Goods側の既存残高を単純に振り替えても通貨保存の前提はまだ成立しない。
