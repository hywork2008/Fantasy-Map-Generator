...
| Nobility | `Characters.advanceAge` → `assignOfficers`/`assignProvinceLords` → （月初）`Mobilization.conscript` → `Espionage.generate()` → `Military.updateDynamic()` → `advanceAllRegimentMovement()` → `refreshCharactersOverviewIfOpen()`。`worldContext.options.conflictAutonomy !== "playerDirected"` のときだけ、月初の `StrategicPlanner.evaluatePlans()`、戦略目標の生成・緊張進行・包囲、小競り合い、戦略目標への進軍、進軍中の占領・略奪も実行する。Player-directed では経済・人口・人物・徴募の時間進行を保ったまま、これらの自律紛争経路を停止する。 | `src/extensions/nobility/index.tsx` |
