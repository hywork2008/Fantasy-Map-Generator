...
> **移行予定（2026-08-12）**: 上の年平均式は現在実装済みの v1 である。季節別作物暦の導入時は、主食・果樹・牧畜を同じ月次労働配列へ合算し、そのピークから `minimumFarmAdults`、`farmLaborRequired`、`migratableAdults`、`ruralReleasePressure`、`laborAffordableCultivatedArea` を一度だけ導出する。年平均式を追加控除として残さない。詳細は [季節別作物暦・農繁期・混合農業労働](../plan/seasonal-crop-calendars.md) を参照。
...
> **移行予定（2026-08-12）**: 以下は現在実装済みの暫定挙動である。緯度・作物・気候ごとの収穫回数と農繁期を実装する計画は [季節別作物暦・農繁期・混合農業労働](../plan/seasonal-crop-calendars.md) を参照。移行後はこの地図全体の補正を廃止し、赤道横断地図にも対応する少数の季節地域と、生成時にセルへ割り当てる農業気候ゾーン別・作物別の月次作物暦を Food Ledger と労働配分器の唯一の入力にする。
