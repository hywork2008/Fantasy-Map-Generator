...
`temp/map_parts`フォルダ以下の.mapファイルを展開した元の`temp/Auteia 2026-07-08-03-32.map`をLoadして+10年進めると、各国の`1st Fleet`が攻撃されていないのか、ずっと海岸沿いの都市を簡単に占領して回り、しかも元の所有国の軍隊はずっと近くにいるのに防衛出来なかっただけでなく、敵国の所属になった海岸沿いの都市を取り戻す事も出来ない状態になっています。

Fleetは恐らく大砲が前提のPowerになっている。
しかし本プロジェクトでは研究開発での解禁があるまでは銃火器は出さない方針であり、艦船は人を移動させるだけの輸送船としての役割しか無い。
また、操船に必要な人員と、運ぶ為に乗せている軍団分の戦闘力しか無い想定。
使える船の数は既存の船+Shipyardsで作成した船の数で、基本的には海路でないとたどり着けないところに軍隊を送ったり、海路を塞いで敵の艦船の侵入を防ぐ警備の役割しか無い想定。

そもそも都市の人口に対して極端に人数の少ない占領軍という構図はおかしい。

## 移動・時間粒度・編成の再設計（引き継ぎ）

騎馬ではない陸軍が1日に24-32km、騎馬隊がずっとでは無いですが倍の48-64km移動出来ると仮定すると現在のAdvance Time 1年あればどこへでも行けてしまう。戦争の為に年月日の概念を導入し、1日ごとのTickを用意し、軍隊が常識的な範囲内で移動する事を確認する必要がある。陸軍が敵地に繋がらない半島に潜り込まないようにpath findingをする必要もある。敵味方それぞれが状況に応じて軍隊を動かすには現在の部隊の上限数では足りないはずで、小隊・中隊・大隊・旅団・師団など適切に分けたり合流し、お互いの侵攻の意図を推測し動く機能、目視で確認したなら寄っていくか防衛の為に都市に入る事も検討しなければいけない、との指摘。

**未着手**（実装計画のみ）。詳細な調査結果・設計の叩き台・次にやるべきことは `docs/plan/military-movement.md` に切り出した。次のセッションはこのメモではなくそちらから着手する。
