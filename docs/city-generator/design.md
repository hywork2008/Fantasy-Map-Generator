...
5. 「Grid evolution」スライダーは scatter → Lloyd1（確定）の可視化。
...
| **M7 ✅** | S7 敷地（TownGeneratorTS 2.6、§4.2）── `core/buildings.ts` 新設。セル辺を壁 / 広場 / 動脈 / 河なら `MAIN_STREET/2`、市内なら `REGULAR_STREET/2`、郊外なら `ALLEY/2` で inset。凸は `shrinkPolygon`（半平面クリップ）、凹は `bufferPolygon`（頂点オフセット）。ブロックを地区ごとに最長辺で再帰二分割（`createAlleys`、カットに路地ギャップ）。Castle / Cathedral は粗い直交分割、Plaza は像 1 つ、空 Ward は建物なし、囲まれていない区画は `filterOutskirts`。Snapshot `S7 · Lots` は都市セルをストリート色にし建物を重ねるので **セル辺が街路として見える**。決定論（`${seed}:program:lots`） | `tsc` 0、`vitest` 153/153（`buildings.test.ts` 8: 凸 shrink・凹 buffer、空 Ward は建物なし、壁/市内/郊外のセットバック ≥ 半街路幅、Plaza は void、建物の 90%+ が自セル内、決定論、S7 が最終ステップ。`pipeline.test.ts`／`synthSite.matrix.test.ts` を 8 ステップへ更新）、biome クリーン。ブラウザ実測（dev、standalone smallCity）: 既定が S7 · Lots、城壁内に建物とセットバック街路、Plaza void、Inspector に building、S6↔S7 で建物が出入り、Citadel で castle 棟、console エラー無し。build: city チャンク 91.5 KB raw / 32.0 KB gzip（M6 から +8.2 KB）、world/d3/three 参照 0 |
