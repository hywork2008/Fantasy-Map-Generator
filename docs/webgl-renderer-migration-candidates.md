...
- **(修正)** 上記の `webglMapTexture.ts` 経由の地形テクスチャ生成には色バグがあった。オンスクリーンの deck instance をコピーできない場合（=ユーザーが `webglHybrid` ではなく `svg` レンダーモードのとき常に発生）、独自に生成した raw WebGL2 context を `gl:` prop 経由で新しい `Deck` に渡すフォールバック経路を通っていたが、この経路だけ land ポリゴンの色が正しい入力値（CPU 側の `fillColor` は検証済みで正しい）に対し ~15% 暗く描画された（MaskExtension の有無に関わらず再現）。viewGlobe の `updateGlobeTexure()` は同じ状況で `getMapURL("mesh", {...})` による SVG snapshot texture を使っており色が正しかったため、`three-d-renderer.ts` の `createMeshTexture()` も同じ方式に切り替え、`webglMapTexture.ts` を削除した（`buildDeckLayers()` 自体は `webglHybrid` の画面描画に引き続き使われるので変更なし）。`getMapURL` に `noRoutes` オプションを追加し、mesh 用途では `noLabels`/`noRoutes` で 3D シーン側が別描画する要素を除外する。
- **(追加修正)** 上記の切り替え後も viewMesh の地形が viewGlobe/standard より暗く見える報告が続いた。原因は `createMeshTexture()` が新しい `CanvasTexture` に `texture.colorSpace = THREE.SRGBColorSpace` を明示していたのに対し、`updateGlobeTexure()` 側の同種の `CanvasTexture` はデフォルト（未指定）のままだったこと。両方とも同じ `getMapURL("mesh", {...})` 由来の SVG ラスタライズ画像を描画元にしているため、この colorSpace タグの有無だけが差分になっていた。`createMeshTexture()` からこの明示的なタグ付けを削除し、globe と同じ「未指定のまま `Renderer.outputColorSpace = THREE.SRGBColorSpace` に任せる」形に揃えたところ、州の塗り色などが Standard / Mesh 間でピクセル単位で完全一致するようになった。
