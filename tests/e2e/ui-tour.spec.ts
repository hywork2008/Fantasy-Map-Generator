...
async function startTour(page: Page) {
  await page.waitForFunction(() => typeof (window as any).fmg?.startUITour === "function", {
    timeout: 15000,
  });
  await page.evaluate(() => (window as any).fmg.startUITour());
...
test.describe("UI Tour", () => {
  ...
  test("UITour is registered as window.fmg.startUITour", async ({ page }) => {
    const ok = await page.evaluate(
      () =>
        typeof (window as any).fmg === "object" &&
        typeof (window as any).fmg.startUITour === "function",
    );
    expect(ok).toBe(true);
  });
