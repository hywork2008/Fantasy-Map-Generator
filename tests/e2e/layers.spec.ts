...
test.describe("map layers", () => {
  ...
  test("labels layer", async () => {
    ...
    // Remove text content but keep structure (text rendering varies across environments)
  ...
  test("labels group can be hidden with display:none", async () => {
    ...
    // Wait for the group select to be populated by the change handler
    await sharedPage.waitForFunction(
      () =>
        (document.getElementById("styleGroupSelect") as HTMLSelectElement | null)
          ?.options.length! > 0
    );

    await sharedPage.evaluate(() => {
      const styleGroupSelect = document.getElementById(
        "styleGroupSelect"
      ) as HTMLSelectElement;
      styleGroupSelect.value = "states";
      styleGroupSelect.dispatchEvent(new Event("change", { bubbles: true }));
    });
    // Wait for the select value to be applied
    await sharedPage.waitForFunction(
      () =>
        (document.getElementById("styleGroupSelect") as HTMLSelectElement | null)
          ?.value === "states"
    ...
    // Apply hide via D3 (same as the checkbox handler does internally)
    ...
    const inlineDisplay = await statesGroup.evaluate(
      (el) => (el as SVGGElement).style.display
    );
