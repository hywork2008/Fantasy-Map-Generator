#!/usr/bin/env node
/**
 * Bake the Arabia EarthRegion raster: Natural Earth land from Croatia
 * (NW) to Sri Lanka (SE), with a coastal-sea margin south and east of
 * Sri Lanka. Plus GMTED/ETOPO elevation.
 *
 * Usage: node scripts/bakeArabiaEarthRaster.mjs
 */
...
const TILE_DIR = "/tmp/terrarium-arabia";
const NE_PATHS = [
...
async function main() {
  ...
  const dest = path.join(OUT_DIR, "arabia.bin");
