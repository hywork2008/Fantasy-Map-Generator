#!/usr/bin/env node
/**
 * Bake the East Asia EarthRegion raster (land mask + GMTED/ETOPO elevation)
 * from Natural Earth 10m countries and AWS terrarium tiles.
 *
 * Usage: node scripts/bakeEastAsiaEarthRaster.mjs
 */
...
const TILE_DIR = "/tmp/terrarium-east-asia";
const NE_PATHS = [
...
function loadCountries() {
  ...
  throw new Error("Natural Earth 10m countries GeoJSON not found. Place it at temp/ne_10m_admin_0_countries.geojson");
}
...
function rasterizeLand(features) {
  ...
  console.log("rasterized polygons", polyCount, "land px", land.reduce((s, v) => s + v, 0));
  return land;
}
...
async function main() {
  ...
  const { tiles, z } = await loadTiles(6);
  ...
  const dest = path.join(OUT_DIR, "east-asia.bin");
