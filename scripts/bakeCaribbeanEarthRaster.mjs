#!/usr/bin/env node
/**
 * Bake the Caribbean EarthRegion raster: Natural Earth land from
 * Los Angeles (NW) to Belém (SE), plus GMTED/ETOPO elevation.
 *
 * Usage: node scripts/bakeCaribbeanEarthRaster.mjs
 */
...
const TILE_DIR = "/tmp/terrarium-caribbean";
const NE_PATHS = [
...
async function main() {
  ...
  const dest = path.join(OUT_DIR, "caribbean.bin");
