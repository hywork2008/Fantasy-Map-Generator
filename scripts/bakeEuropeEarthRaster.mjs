#!/usr/bin/env node
/**
 * Bake the Europe EarthRegion raster: Natural Earth land from Ireland
 * to Georgia and Cyprus to Finland (EU members + candidates + applicants),
 * plus GMTED/ETOPO elevation.
 *
 * Usage: node scripts/bakeEuropeEarthRaster.mjs
 */
...
const TILE_DIR = "/tmp/terrarium-europe";
const NE_PATHS = [
...
async function main() {
  ...
  const dest = path.join(OUT_DIR, "europe.bin");
