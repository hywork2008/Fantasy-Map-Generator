#!/usr/bin/env node
/**
 * Bake the Mediterranean Sea EarthRegion raster: Natural Earth land from
 * the Atlantic approach to Gibraltar through the Levant, plus GMTED/ETOPO.
 *
 * Usage: node scripts/bakeMediterraneanSeaEarthRaster.mjs
 */
...
const TILE_DIR = "/tmp/terrarium-mediterranean";
const NE_PATHS = [
...
async function main() {
  ...
  const dest = path.join(OUT_DIR, "mediterranean-sea.bin");
