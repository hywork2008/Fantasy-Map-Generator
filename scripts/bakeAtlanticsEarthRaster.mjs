#!/usr/bin/env node
/**
 * Bake the Atlantics EarthRegion raster: Natural Earth land on the
 * North Atlantic basin (complete North America and Europe, plus
 * Greenland / Maghreb / northern South America) and GMTED/ETOPO.
 *
 * Usage: node scripts/bakeAtlanticsEarthRaster.mjs
 */
...
const TILE_DIR = "/tmp/terrarium-atlantics";
const NE_PATHS = [
...
async function main() {
  ...
  const dest = path.join(OUT_DIR, "atlantics.bin");
