#!/usr/bin/env node
/**
 * Bake the Europe Central EarthRegion raster: Natural Earth land in the
 * Industrial Revolution core (Channel / Low Countries / Rhine / Elbe)
 * plus GMTED/ETOPO elevation.
 *
 * Usage: node scripts/bakeEuropeCentralEarthRaster.mjs
 */
...
const TILE_DIR = "/tmp/terrarium-europe-central";
const NE_PATHS = [
...
async function main() {
  ...
  const dest = path.join(OUT_DIR, "europe-central.bin");
