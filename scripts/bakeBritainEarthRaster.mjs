#!/usr/bin/env node
/**
 * Bake the Britain EarthRegion raster: Natural Earth land in the British
 * Isles theatre (Ireland to Shetland, plus in-frame neighbors) and
 * GMTED/ETOPO elevation.
 *
 * Usage: node scripts/bakeBritainEarthRaster.mjs
 */
...
const TILE_DIR = "/tmp/terrarium-britain";
const NE_PATHS = [
...
async function main() {
  ...
  const dest = path.join(OUT_DIR, "britain.bin");
