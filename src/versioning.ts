...
export function parseMapVersion(version: string): string {
  ...
  // e.g. 0.7b
  const majorNum = parseInt(major, 10) || 0;
  const minorNum = parseInt(minor, 10) || 0;
  const patchNum = parseInt(patch, 10) || 0;
...
export function compareVersions(
  version1: string,
  version2: string,
  options: CompareVersionsOptions = { major: true, minor: true, patch: true }
