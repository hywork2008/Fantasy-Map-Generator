...
export const BordersRenderer: IRenderer = {
  ...
  render(worldContext: Readonly<WorldContext>, viewContext: Readonly<RootLayers>, _appServices: AppServices): void {
    ...
    svg.select<SVGGElement>("#borders").attr("fill", "none").selectAll("path").remove();
    svg.select<SVGGElement>("#stateBorders").append("path").attr("d", statePath.join(" "));
    svg.select<SVGGElement>("#provinceBorders").append("path").attr("d", provincePath.join(" "));
