...
/**
 * Re-renders the same `cachedOceanPaths` (generated by the last `OceanLayers()` call) to an
 * offscreen HTMLCanvasElement. This canvas is used by the WebGL hybrid renderer's BitmapLayer
 * so that the depth-gradient appearance is pixel-identical to SVG mode.
 *
 * Returns `null` when no paths have been generated yet (before the first map draw).
 */
export function renderOceanDepthToOffscreenCanvas(graphWidth: number, graphHeight: number): HTMLCanvasElement | null {
  if (cachedOceanPaths.length === 0) return null;
...
/** The number of path entries in the current ocean-layers cache (used as an invalidation token). */
export function getOceanPathsCacheSize(): number {
  return cachedOceanPaths.length;
}
