...
function editProvinces(): void {
  ...
  body.addEventListener("click", (ev: MouseEvent) => {
    ...
    else if (cl.contains("icon-dot-circled")) overviewBurgs({ stateId } as any);
  ...
  function collectStatistics(): void {
    const { cells } = pack;
    const provinces = pack.provinces as any[];
    const burgs = pack.burgs as any[];
  ...
  function updateFilter(): void {
    ...
    const statesSorted = (pack.states as any[])
      .filter((s: any) => s.i && !s.removed)
      .sort((a: any, b: any) => (a.name > b.name ? 1 : -1));
    statesSorted.forEach((s: any) => {
  ...
  function provincesEditorAddLines(): void {
    ...
    let filtered = (pack.provinces as any[]).filter((p: any) => p.i && !p.removed);
    ...
    for (const p of filtered) {
      ...
      const stateName = (pack.states as any[])[p.state].name;
      const capital = p.burg ? (pack.burgs as any[])[p.burg].name : "";
      const separable = p.burg && p.burg !== (pack.states as any[])[p.state].capital;
      ...
      >
        ...
        <select data-tip="Province capital. Click to select from burgs within the state. No capital means the province is governed from the state capital" class="cultureBase hide ${p.burgs.length ? "" : "placeholder"}">
  ...
  function getCapitalOptions(burgs: number[], capital: number): string {
    let options = "";
    burgs.forEach(b => {
      options += `<option ${b === capital ? "selected" : ""} value="${b}">${(pack.burgs as any[])[b].name}</option>`;
    });
    return options;
  }
  ...
  function changeFill(el: HTMLElement): void {
    ...
    const callback = (newFill: string) => {
      ...
      (pack.provinces as any[])[p].color = newFill;
  ...
  function capitalZoomIn(p: number): void {
    const capital = (pack.provinces as any[])[p].burg;
  ...
  function declareProvinceIndependence(provinceId: number): [number, number] {
    ...
    if ((provinceBurgs as number[]).some((b: number) => (burgs as any[])[b].capital))
      return tip(
        "Cannot declare independence of a province having capital burg. Please change capital first",
        false,
        "error"
      ) as any;
    if (!burgId) return tip("Cannot declare independence of a province without burg", false, "error") as any;

    const oldStateId = province.state;
    const newStateId = (states as any[]).length;

    const capital = (burgs as any[])[burgId];
    capital.capital = 1;
    Burgs.changeGroup(capital);

    (provinceBurgs as number[]).forEach((b: number) => {
      (burgs as any[])[b].state = newStateId;
    });

    const { cell: center, culture } = (burgs as any[])[burgId];
    ...
    const diplomacy = (states as any[]).map((s: any) => {
      ...
      let relations = (states as any[])[oldStateId].diplomacy[s.i];
    ...
    (states as any[])[0].diplomacy.push([
      ...
      `${name} declared its independance from ${(states as any[])[oldStateId].name}`
    ]);
    ...
    (states as any[])[oldStateId].provinces = (states as any[])[oldStateId].provinces.filter(
      (p: number) => p !== provinceId
    );
  ...
  function updateStatesPostRelease(oldStates: number[], newStates: number[]): void {
    ...
    allStates.forEach(stateId => {
      ...
      const { coa, pole } = (pack.states as any[])[stateId];
  ...
  function changePopulation(province: number): void {
    const p = (pack.provinces as any[])[province];
    ...
    function applyPopulationChange(): void {
      ...
      if (Number.isFinite(urbanChange) && urbanChange !== 1) {
        p.burgs.forEach((b: number) => {
          (pack.burgs as any[])[b].population = rn((pack.burgs as any[])[b].population * urbanChange, 4);
        });
      }
      if (!Number.isFinite(urbanChange) && urbanPopEl().valueAsNumber > 0) {
        ...
        p.burgs.forEach((b: number) => {
          (pack.burgs as any[])[b].population = population;
        });
      }
    ...
    ($("#alert") as any).dialog({
      ...
      buttons: {
        Remove: function (this: Element) {
          ...
          const s = (pack.provinces as any[])[p].state;
          const state = (pack.states as any[])[s];
  ...
  function editProvinceName(province: number): void {
    ...
    ensureEl("provinceCultureDisplay").innerText = (pack.cultures as any[])[cultureId].name;
    ...
    function regenerateShortNameCulture(): void {
      ...
      const culture = pack.cells.culture[(pack.provinces as any[])[prov].center];
    ...
    function applyNameChange(p: any): void {
  ...
  function changeCapital(p: number, line: HTMLElement, value: string): void {
    line.dataset.capital = (pack.burgs as any[])[+value].name;
    (pack.provinces as any[])[p].center = (pack.burgs as any[])[+value].cell;
    (pack.provinces as any[])[p].burg = +value;
  }
  ...
  function showChart(): void {
    const getClr = (s: any) =>
      !s.i || s.removed || (s.color as string)[0] !== "#" ? "#666" : color(s.color)?.darker();
    const states = (pack.states as any[]).map(s => ({ id: s.i, state: s.i ? 0 : null, color: getClr(s) }));
    const provinces = (pack.provinces as any[])
    ...
    const treeLayout = (d3 as any).treemap().size([w, h]).padding(2);
    ...
    function showInfo(ev: MouseEvent, d: any): void {
      (d3 as any).select(ev.target).select("rect").classed("selected", 1);
      const name = d.data.fullName;
      const state = (pack.states as any[])[d.data.state].fullName;
    ...
    function hideInfo(ev: MouseEvent): void {
      ...
      (d3 as any).select(ev.target).select("rect").classed("selected", 0);
    }
    ...
    confirmationDialog({
      ...
      onConfirm: () => {
        ...
        body.querySelectorAll<HTMLElement>(":scope > div").forEach(el => {
          ...
          const province = (pack.provinces as any[])[provinceId];
          if (!province.burg) return;
          if (province.burg === (pack.states as any[])[province.state].capital) return;
          if ((province.burgs as number[]).some((burgId: number) => (pack.burgs as any[])[burgId].capital)) return;
  ...
  function changeForSelection(selection: number[]): void {
    ...
    const state = (pack.provinces as any[])[provinceNew].state;
    const fill = (pack.provinces as any[])[provinceNew].color || "#ffffff";
    ...
    selection.forEach(i => {
      ...
      if (i === (pack.provinces as any[])[provinceOld]?.center) {
  ...
  function applyProvincesManualAssignment(): void {
    provs
      ...
      .each(function () {
        ...
        const i = +(el as any).dataset.cell;
        pack.cells.province[i] = +(el as any).dataset.province;
      });
  ...
  function enterAddProvinceMode(this: HTMLButtonElement): void {
    ...
    viewbox.style("cursor", "crosshair").on("click", addProvince as any);
    body.querySelectorAll<HTMLElement>("div > input, select, span, svg").forEach(e => {
      e.style.pointerEvents = "none";
    });
  }
  ...
  function addProvince(this: SVGElement, event: MouseEvent): void {
    ...
    if (oldProvince && (provinces as any[])[oldProvince].center === center) {
    ...
    const province = (provinces as any[]).length;
    (pack.states as any[])[state].provinces.push(province);
    const burg = cells.burg[center];
    const c = cells.culture[center];
    const name = burg ? (pack.burgs as any[])[burg].name : Names.getState(Names.getCultureShort(c), c);
    const formName = oldProvince ? (provinces as any[])[oldProvince].formName : "Province";
    const fullName = `${name} ${formName}`;
    const stateColor = (pack.states as any[])[state].color;
    const rndColor = getRandomColor();
    const newColor = stateColor[0] === "#" ? color(interpolate(stateColor, rndColor)(0.2))!.formatHex() : rndColor;

    const kinship = burg ? 0.8 : 0.4;
    const parent = burg ? (pack.burgs as any[])[burg].coa : (pack.states as any[])[state].coa;
    ...
    COArenderer.add("province", province, coa as any, px, py);

    (provinces as any[]).push({ i: province, state, center, burg, name, formName, fullName, color: newColor, coa });

    cells.province[center] = province;
    (cells.c[center] as unknown as number[]).forEach((cc: number) => {
      if (cells.h[cc] < 20 || cells.state[cc] !== state) return;
      if ((provinces as any[]).find((p: any) => !p.removed && p.center === cc)) return;
  ...
  function recolorProvinces(): void {
    ...
    (pack.provinces as any[]).forEach(p => {
      if (!p || p.removed) return;
      if (state !== -1 && p.state !== state) return;
      const stateColor = (pack.states as any[])[p.state].color;
      const rndColor = getRandomColor();
  ...
  function downloadProvincesData(): void {
    ...
    body.querySelectorAll<HTMLElement>(":scope > div").forEach(el => {
      ...
      const provincePack = (pack.provinces as any[])[key];
      data += `${el.dataset.id},`;
  ...
  function openProvinceMergeDialog(): void {
    ...
    const provincesToMerge = (pack.provinces as any[]).filter(p => p.i && !p.removed && p.state === selectedState);
          ...
          confirmationDialog({
            ...
            message: `
              <p>The following provinces will be <strong>removed</strong>: ${provincesToMergeIds
                .map(provinceId => `${emblem(provinceId)}${(pack.provinces as any[])[provinceId].name}`)
                .join(", ")}.</p>
              <p>Removed provinces data (burgs and cells) will be assigned to ${emblem(primaryProvinceId)}${(pack.provinces as any[])[primaryProvinceId].name}.</p>
  ...
  function mergeProvinces(ids: number[], primary: number): void {
    const primaryProvince = (pack.provinces as any[])[primary];
    const provinceIdMap = new Map<number, number>();

    ids.forEach(id => {
      if (id === primary) return;
      const province = (pack.provinces as any[])[id];

      (province.burgs as number[]).forEach((b: number) => {
        (pack.burgs as any[])[b].province = primary;
      ...
      (pack.provinces as any[])[id] = { i: id, removed: true };
    });
    ...
    const state = (pack.states as any[])[primaryProvince.state];
    state.provinces = (state.provinces as number[]).filter((p: number) => !(pack.provinces as any[])[p].removed);
...
function updateLockStatus(provinceId: number, classList: DOMTokenList): void {
  const p = (pack.provinces as any[])[provinceId];
  p.lock = !p.lock;
