...
export function overviewRegiments(state = -1): void {
  ...
  function addLines(): void {
    ...
    for (const el of body.querySelectorAll("div.states"))
      el.addEventListener("mouseenter", ev => regimentHighlightOn(ev as MouseEvent));
    for (const el of body.querySelectorAll("div.states"))
      el.addEventListener("mouseleave", ev => regimentHighlightOff(ev as MouseEvent));
  }

  function updateFilter(stateId: number): void {
  ...
  function togglePercentageMode(): void {
    if (body.dataset.type === "absolute") {
      ...
      const total = (type: string): number => {
        ...
        cache[type] = sum(array.map(el => +(el as HTMLElement).dataset[type]! || 0));
        return cache[type];
      };

      lines.forEach(el => {
        el.querySelectorAll("div").forEach(div => {
          const type = (div as HTMLElement).dataset.type!;
          if (type === "rate") return;
          const t = total(type);
          div.textContent = t ? `${rn((+(el as HTMLElement).dataset[type]! / t) * 100)}%` : "0%";
        });
      });
    } else {
      body.dataset.type = "absolute";
