...
class RegionalWaterAuthorityModule {
  ...
  generate(): void {
    setRegionalWaterSchemes([]);
    setAnnualGateYear(ANNUAL_GATE.regionalWaterSchemes, getSimulationYear());
  }

  clear(): void {
    setRegionalWaterSchemes([]);
    setAnnualGateYear(ANNUAL_GATE.regionalWaterSchemes, -1);
  }
  ...
  settleAnnual(): boolean {
    if (!settleAnnualOnce(ANNUAL_GATE.regionalWaterSchemes)) return false;
