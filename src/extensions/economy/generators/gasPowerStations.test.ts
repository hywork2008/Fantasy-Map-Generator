import { afterEach, beforeEach, describe, expect, it } from "vitest";
import { setTechnologyProgressForTests } from "../../../generators/technologyProgress";
import { worldContext } from "../../hostCore";
import type { ExtensionAPI, PackedGraph } from "../../hostTypes";
import { rn } from "../../hostUtils";
import {
  clearEconomyContext,
  getCraftDomainEmploymentRecords,
  getGasPowerStations,
  getMarkets,
  initEconomyContext,
  setGoods,
  setMarkets
} from "../economyContext";
import { FACILITY_MAINTENANCE_RATE, GAS_POWER_STATION_BUDGET } from "./chemMedCommon";
import { GAS_POWER_STATION_BASE_CAPACITY, GasPowerStations } from "./gasPowerStations";
import { Goods } from "./goods-generator";
import { Markets } from "./markets-generator";

describe("GasPowerStationsModule", () => {
  beforeEach(() => {
    initEconomyContext({ worldContext } as unknown as ExtensionAPI);
    worldContext.options = { year: 1920 } as typeof worldContext.options;
    worldContext.pack = {
      states: [{ i: 0 }, { i: 1, name: "Gasland", removed: false, capital: 1, treasury: 200 }],
      burgs: [{ i: 0 }, { i: 1, state: 1, market: 1, capital: 1, x: 0, y: 0, removed: false, sanitation: 50 }]
    } as unknown as PackedGraph;
    setGoods([
      { i: 1, name: "LNG", tags: [], value: 12, unit: "therm", icon: "good-unknown", color: "#cfe8f0" },
      { i: 2, name: "Copper Wire", tags: [], value: 16, unit: "coil", icon: "good-unknown", color: "#c98a4b" },
      { i: 3, name: "Machine Parts", tags: [], value: 18, unit: "crate", icon: "good-unknown", color: "#6d7380" }
    ]);
    setMarkets([
      {
        i: 1,
        centerBurgId: 1,
        color: "#111",
        goods: {
          1: { stock: 100, price: 12 },
          2: { stock: 100, price: 16 },
          3: { stock: 100, price: 18 }
        }
      }
    ]);
    Goods.sync();
    Markets.sync();
  });

  afterEach(() => {
    clearEconomyContext();
    setTechnologyProgressForTests([]);
  });

  it("does not create a plant for a State where gasFiredElectricityGeneration has not reached known", () => {
    setTechnologyProgressForTests([]);
    expect(GasPowerStations.settleAnnual()).toBe(true);
    expect(getGasPowerStations()).toHaveLength(0);
    expect(worldContext.pack.states[1].treasury).toBe(200);
  });

  it("creates a plant, debits the budget, consumes LNG/Copper Wire/Machine Parts, computes generationCapacity, and grows instruments once known", () => {
    setTechnologyProgressForTests([
      { technologyId: "gasFiredElectricityGeneration", scope: "state", ownerId: 1, stage: "known", diffusion: 0 }
    ]);

    expect(GasPowerStations.settleAnnual()).toBe(true);

    const plants = getGasPowerStations();
    expect(plants).toHaveLength(1);
    expect(plants[0]).toMatchObject({ stateId: 1, role: "trial", active: true, utilization: 1, documentedRuns: 1 });
    // Trial-role capacity: GAS_POWER_STATION_BASE_CAPACITY * 0.25 * utilization(1).
    expect(plants[0]?.generationCapacity).toBe(GAS_POWER_STATION_BASE_CAPACITY * 0.25);
    // Same double-debit shape as PowerStations: one charge to found, one for this year's run.
    // One full charge to found the plant, one reduced FACILITY_MAINTENANCE_RATE renewal charge for
    // this year's operation (docs/plan/treasury-structural-deficit-investigation.md §8.2, fix "A").
    expect(worldContext.pack.states[1].treasury).toBe(
      200 - GAS_POWER_STATION_BUDGET - rn(GAS_POWER_STATION_BUDGET * FACILITY_MAINTENANCE_RATE, 2)
    );

    const market = getMarkets().find(entry => entry.i === 1);
    expect(market?.goods[1]?.stock).toBe(100 - 3); // LNG consumed
    expect(market?.goods[2]?.stock).toBe(100 - 1); // Copper Wire consumed
    expect(market?.goods[3]?.stock).toBe(100 - 1.5); // Machine Parts consumed

    const instruments = getCraftDomainEmploymentRecords().find(row => row.burgId === 1 && row.domain === "instruments");
    expect(instruments?.workers).toBe(2);
  });

  it("reduces utilization and leaves generationCapacity at 0 when LNG stock is scarce", () => {
    setMarkets([
      {
        i: 1,
        centerBurgId: 1,
        color: "#111",
        goods: {
          1: { stock: 0.5, price: 12 },
          2: { stock: 100, price: 16 },
          3: { stock: 100, price: 18 }
        }
      }
    ]);
    Markets.sync();
    setTechnologyProgressForTests([
      { technologyId: "gasFiredElectricityGeneration", scope: "state", ownerId: 1, stage: "known", diffusion: 0 }
    ]);

    expect(GasPowerStations.settleAnnual()).toBe(true);

    const plants = getGasPowerStations();
    expect(plants[0]?.utilization).toBeLessThan(0.5);
    expect(plants[0]).toMatchObject({
      documentedRuns: 0,
      generationCapacity: 0,
      lastFailureReason: "materialShortage"
    });
  });

  it("marks the plant inactive with fundingCut when the State cannot afford the annual budget", () => {
    worldContext.pack.states[1].treasury = GAS_POWER_STATION_BUDGET; // enough to found, not to operate
    setTechnologyProgressForTests([
      { technologyId: "gasFiredElectricityGeneration", scope: "state", ownerId: 1, stage: "known", diffusion: 0 }
    ]);

    expect(GasPowerStations.settleAnnual()).toBe(true);

    const plants = getGasPowerStations();
    expect(plants[0]).toMatchObject({ active: false, generationCapacity: 0, lastFailureReason: "fundingCut" });
    expect(worldContext.pack.states[1].treasury).toBe(0);
  });

  it("promotes a trial plant to service once gasFiredElectricityGeneration reaches adopted", () => {
    setTechnologyProgressForTests([
      { technologyId: "gasFiredElectricityGeneration", scope: "state", ownerId: 1, stage: "known", diffusion: 0 }
    ]);
    expect(GasPowerStations.settleAnnual()).toBe(true);
    expect(getGasPowerStations()[0]?.role).toBe("trial");
    expect(getGasPowerStations()[0]?.generationCapacity).toBe(GAS_POWER_STATION_BASE_CAPACITY * 0.25);

    worldContext.options = { year: 1921 } as typeof worldContext.options;
    setTechnologyProgressForTests([
      { technologyId: "gasFiredElectricityGeneration", scope: "state", ownerId: 1, stage: "adopted", diffusion: 0 }
    ]);
    expect(GasPowerStations.settleAnnual()).toBe(true);
    expect(getGasPowerStations()[0]?.role).toBe("service");
    // Full-scale capacity once promoted to service (role factor 1 instead of 0.25).
    expect(getGasPowerStations()[0]?.generationCapacity).toBe(GAS_POWER_STATION_BASE_CAPACITY);
  });

  it("self-gates to once per simulation year", () => {
    setTechnologyProgressForTests([
      { technologyId: "gasFiredElectricityGeneration", scope: "state", ownerId: 1, stage: "known", diffusion: 0 }
    ]);
    expect(GasPowerStations.settleAnnual()).toBe(true);
    expect(GasPowerStations.settleAnnual()).toBe(false);
  });
});
