...
export function open(): void {
  ...
  for (const def of INPUTS) {
    ...
    const currentOptions = TradeAnimation.getOptions();
    const current = currentOptions[key] ?? def.default;
    ...
    input.addEventListener("input", () => {
      ...
      TradeAnimation.configure({ [key]: value });
      if (output) output.textContent = String(value);
      localStorage.setItem("trade-animation", JSON.stringify(TradeAnimation.getOptions()));
      TradeAnimation.restart();
    });

    resetBtn.addEventListener("click", () => {
      TradeAnimation.configure({ [key]: def.default });
...
function buildDialogHTML(): string {
  const rows = INPUTS.map(({ id, label, type, selectOptions, tip, min, max, step, key, default: def }) => {
    const currentOptions = TradeAnimation.getOptions();
    const current = currentOptions[key as keyof typeof currentOptions] ?? def;
    const input =
