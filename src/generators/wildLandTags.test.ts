...
describe("assignWildLandTags", () => {
  it("writes tags and forbids outposts only on monster domains", () => {
    const cells = {
    ...
    expect(allowsFrontierOutpost(tags[1])).toBe(true);
    expect(allowsFrontierOutpost(WILD_LAND.monster)).toBe(false);
  });
});
