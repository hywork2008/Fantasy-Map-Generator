 ...
 * @param {Array<string | number>} data - Array of data values corresponding to each cell
 * @param {PackedGraph} packedGraph - The packed graph object containing cell positions
 */
export const drawCellsValue = (data: Array<string | number>, packedGraph: PackedGraph): void => {
  ...
  window.debug
    ...
    .enter()
    .append("text")
    .attr("x", (_d: string | number, i: number) => packedGraph.cells.p[i][0])
    .attr("y", (_d: string | number, i: number) => packedGraph.cells.p[i][1])
 ...
 * @param {Selection<SVGGElement, unknown, null, undefined>} terrs - The SVG group element where the polygons will be drawn
 */
 ...
 * @param {PackedGraph} packedGraph - The packed graph object containing cell positions and routes
 */
export const drawRouteConnections = (packedGraph: PackedGraph): void => {
