...
const { Features, Cultures, Burgs, Markers, Provinces } = autoUpdateFmgApi;
...
export function runAutoUpdatePostV110Migrations(mapVersion, context: AutoUpdateContext) {
  ...
  if (isOlderThan("1.21.0")) {
    ...
    rivers.selectAll("path").each(function () {
      ...
      const source = findPackCell(pack, s.x, s.y);
      const mouth = findPackCell(pack, e.x, e.y);
    ...
    for (const river of rivers) {
      ...
      if (node && !river.cells) {
        ...
        for (let i = 0; i <= segments; i++) {
          ...
          const cell = findPackCell(pack, x, y);
    ...
    for (const g of document.querySelectorAll("#viewbox > #routes > g")) {
      ...
      for (const node of g.querySelectorAll("path")) {
        ...
        for (let i = 0; i <= totalLength + 0.1; i += increment) {
          ...
          const cellId = findPackCell(pack, x, y);
  ...
  if (isOlderThan("1.100.0")) {
    ...
    pack.zones = [];
    getZonesLayer().selectAll("g").each(function () {
    ...
    getZonesLayer().style("display", null).selectAll("*").remove();
