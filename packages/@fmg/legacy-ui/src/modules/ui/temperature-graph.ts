...
export function showBurgTemperatureGraph(id: number) {
  ...
  const prec = temperatureGraphRuntime.grid.cells.prec[temperatureGraphRuntime.pack.cells.g[b.cell]];
  ...
  const xscale = temperatureGraphRuntime.d3.scaleTime().domain([startDate, endDate]).range([0, chartWidth]);
  const yscale = temperatureGraphRuntime.d3.scaleLinear().domain([minT, maxT]).range([chartHeight, 0]);

  const tempMean: Point2D[] = [];
  const tempMin: Point2D[] = [];
  const tempMax: Point2D[] = [];
  ...
  temperatureGraphRuntime.$("#alert").dialog({
    title: "Average temperature in " + b.name,
    position: {my: "center", at: "center", of: "svg"}
  });

  function drawGraph() {
    temperatureGraphRuntime.alertMessage.innerHTML = "";
    const getCurve = (data: Point2D[]) => temperatureGraphRuntime.round(
      temperatureGraphRuntime.d3.line().curve(temperatureGraphRuntime.d3.curveBasis)(data),
      2
    );

    const legendSize = 60;
    const chart = temperatureGraphRuntime.d3
      .select("#alertMessage")
      .append("svg")
    ...
    const xGrid = temperatureGraphRuntime.d3.axisBottom(xscale).ticks().tickSize(-chartHeight);
    const yGrid = temperatureGraphRuntime.d3.axisLeft(yscale).ticks(5).tickSize(-chartWidth);

    const grid = chart.append("g").attr("class", "epgrid").attr("stroke-dasharray", "4 1");
    grid.append("g").attr("transform", `translate(${xOffset}, ${chartHeight + yOffset})`).call(xGrid);
    ...
    const xAxis = temperatureGraphRuntime.d3.axisBottom(xscale).ticks().tickFormat(temperatureGraphRuntime.d3.timeFormat("%B"));
    const yAxis = temperatureGraphRuntime.d3.axisLeft(yscale).ticks(5).tickFormat((v: number) => temperatureGraphRuntime.convertTemperature(v));

    const axis = chart.append("g");
    axis.append("g").attr("transform", `translate(${xOffset}, ${chartHeight + yOffset})`).call(xAxis);
    ...
    function printVal(this: SVGPathElement) {
      const [x, y] = temperatureGraphRuntime.d3.mouse(this);
