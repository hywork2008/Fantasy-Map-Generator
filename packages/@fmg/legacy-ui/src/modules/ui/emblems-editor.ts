...
function editEmblem(type, id, el) {
  ...
  function updateElementSelectors(type, id, el) {
    ...
    stateList.forEach(s => emblemStates.options.add(new Option(s.name, String(s.i), false, s.i === state)));
    ...
    provinceList.forEach(p => emblemProvinces.options.add(new Option(p.name, String(p.i), false, p.i === province)));
    ...
    reader.onload = function (readerEvent) {
      ...
      if (type === "svg") {
        ...
        el.querySelectorAll("*").forEach(el => {
          if (el.id === "adobe_illustrator_pgf") el.remove();
