...
async function checkLoadParameters() {
  ...
  if ((ensureEl("onloadBehavior") as HTMLSelectElement)?.value === "lastSaved") {
    try {
...
function focusOn() {
  ...
  if (scaleParam || cellParam || burgParam) {
    ...
    if (burgParam) {
      const burg = Number.isNaN(+burgParam) ? pack.burgs.find(burg => burg.name === burgParam) : pack.burgs[+burgParam];
      if (!burg) return;
...
function generatePrecipitation() {
  ...
  d3.range(0, cells.i.length, cellsX).forEach((c, i) => {
...
function _isWetLand(moisture, temperature, height) {
...
const regenerateMap = debounce(async options => {
